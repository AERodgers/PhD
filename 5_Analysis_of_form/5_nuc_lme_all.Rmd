---
title: "LME Analysis of NUCs"
author: "Antoin Rodgers"
date: "18.05.2022"
output:
  html_document: default
---

```{r setup, include=F}
knitr::opts_chunk$set(echo = F, 
                      message = F,
                      warning = F)

# Get personal functions and install / load necessary packages.
source("../2_R_Functions/myFunctions.R")

installMissingPackages(
  c(
    # Include statistical packages.
    "performance",
    "lmerTest",
    "lme4",
    "ggeffects",
    "optimx",
    "MuMIn",
    "dfoptim",

    # Include packages for tidy output.
    "tidyverse",
    "broomExtra",
    "sjPlot",
    "formattable",
    "knitr",
    "janitor",
    "gridExtra",
    
    # Include packages for %in% %notin% syntactic notation
    "mefa4"
  )
)

# Set themes and colour schemes.
theme_set(theme_minimal(base_size = 12))

# Get  data for analysis

source("0_get_AH.R")

nuc_data <- nuc %>%
  # there must be a more efficient way of doing this but...
  # Create column of stressed syllables in foot 2.
    select(
    stim,
    speaker,
    gender,
    pre_syls,
    foot_syls,
    nuc_pre_text,
    nuc_new_word,
    nuc_str_syl,
    fin_phon,
    stim,
    h_t,
    h_f0,
    l_t,
    l_f0,
    e_t,
    e_f0,
    f0_exc,
    lh_dur,
    lh_slope,
    foot_dur,
    speech_rate
  )


optimizer = "optimx"

rm(nuc, pn, pn_ana, pn_foot, nuc_pre, nuc_foot, pn_lex, corpus)

```

```{r check levels in factors, results="asis", eval=F}

star_found = F
cat("\n\nFactors for nuc_data\n")
for (i in colnames(nuc_data)) {
  if(length(unique(nuc_data[[i]])) == 1){
    star = " *"
  star_found = T
  }
  else{
    star = ""
    }

  if (is.factor(nuc_data[[eval(i)]])) {
    cat("\n", i, star, "  \tunique = ", length(unique(nuc_data[[i]])),
        "  \tlevels = ", length(levels(nuc_data[[i]])), sep = "")
  }
    else {
    cat("\n", i, sep = "")
    }
}
cat("\n")
if(star_found){
  cat("\n* Factor only has one level in the actual data.\n")
}

nuc_data %>%
  group_by(pre_syls) %>%
  summarise(count = n()) %>% 
  adorn_totals("row") %>% 
  formattable(caption = "pre_syls stim variety")

nuc_data %>%
  group_by(nuc_new_word) %>%
  summarise(count = n()) %>%
  adorn_totals("row") %>% 
  formattable(caption = "nuc_new_word stim variety")
```

## L targets

### Analysis of L timing

```{r run l_t models, fig.height=3, fig.width=9}
l_t_equation = formula(
  l_t ~ foot_syls
  + pre_syls
  + fin_phon
  + nuc_new_word
  + gender
  + (1 | speaker)
  + (1 | nuc_str_syl)
)


nuc_l_t_mdl = lmer(
  l_t_equation,
  nuc_data,
  control = lmerControl(
    optimizer = optimizer,
    calc.derivs = F,
    optCtrl = list(
      method = "nlminb",
      starttests = F,
      kkt = F
    )
  )
)

# model needs trimming
nuc_l_t_data.trimmed <- nuc_data %>% filter(abs(scale(resid(nuc_l_t_mdl))) <= 3)
nuc_l_t_mdl.trimmed = update(nuc_l_t_mdl, data = nuc_l_t_data.trimmed)
 anova <- summariseLME(nuc_l_t_mdl.trimmed,
             run_step = F,
             write = "5_nuc_output/nuc_l_t_anova.csv")
 
 adjustP_posthoc("5_nuc_output/",
                 p.value,
                 suffix_id = "_anova")
 
 read_csv("5_nuc_output/nuc_l_t_anova.csv") %>%
   tidyStatNumbers() %>%
   formattable(caption = paste("ANOVA of model:",
                               getModelFormula(nuc_l_t_mdl.trimmed)))
```

```{r l_t model summary, fig.height=2.5, fig.width=2.823, results="asis"}
re_plot <- plot_model(nuc_l_t_mdl, type = "re")[[2]]

re_plot <- re_plot +
  geom_hline(yintercept = 0, colour = "red") +
  labs(caption = "Random effect of stressed syllable on L timing") +
  ylab("time (ms)") +
  theme(
    plot.title = element_blank(),
    plot.caption = element_text(hjust = 0, size = 9),
    plot.caption.position = "plot"
  )
# re_plot
nuc_l_t_mdl.tidy <- analyseModel(nuc_l_t_mdl,
                                 write = "5_nuc_output/nuc_l_t",
                                 type = "pred",
                                 y_lab = "time (ms)",
                                 y_lim = c(-0, 140),
                                 breaks = seq(-0, 140, 40),
                                 plot_rounding = 0,
                                 panel_prefix = "a. "
                                 )

```

```{r do l_t pairwise analysis, fig.height=2.5, fig.width = 3.01, results = "asis"}
printTidyPredictions(nuc_l_t_mdl)
getModelFixedFX(nuc_l_t_mdl, write="5_nuc_output/nuc_l_t")$slopes %>% 
  mutate(across(
          any_of(c("estimate", "conf.low", "conf.high")),
          ~ round(as.numeric(.),0))) %>% tidyStatNumbers()
```

### Analysis of L F0

```{r set l_f0 equation}
l_f0_equation = formula(
  l_f0 ~
    foot_syls
  + pre_syls
  + fin_phon
  + nuc_new_word
  + gender
  + (1 | speaker)
  + (1 | nuc_str_syl)
)
optimizer = "optimx"
```

```{r run l_f0 models, fig.height=3, fig.width=9}
nuc_l_f0_mdl = lmer(
  l_f0_equation,
  nuc_data,
  control = lmerControl(
    optimizer = optimizer,
    calc.derivs = F,
    optCtrl = list(
      method = "nlminb",
      starttests = F,
      kkt = F
    )
  )
)

# model needs trimming
nuc_l_f0.trimmed <-
  nuc_data %>% filter(abs(scale(resid(nuc_l_f0_mdl))) <= 2.5)

nuc_l_f0_mdl.trimmed = update(nuc_l_f0_mdl, data = nuc_l_f0.trimmed)

anova <- summariseLME(nuc_l_f0_mdl.trimmed,
                      run_step = F,
                      write = "5_nuc_output/nuc_l_f0_anova.csv")

adjustP_posthoc("5_nuc_output/",
                p.value,
                suffix_id = "_anova")

read_csv("5_nuc_output/nuc_l_f0_anova.csv") %>%
  tidyStatNumbers() %>%
  formattable(caption = paste("ANOVA of model:",
                              getModelFormula(nuc_l_f0_mdl.trimmed)))
```

```{r l_f0 model summary, fig.height=2.5, fig.width=2.823, results="asis"}
nuc_l_f0_mdl.tidy <- analyseModel(nuc_l_f0_mdl.trimmed,
                                  write = "5_nuc_output/nuc_l_f0",
                                  type = "pred",
                                  y_lab = "f0 (ST re median)",
                                  y_lim = c(-4.5, 0),
                                  breaks = seq(-4.5, 0, 1),
                                  plot_rounding = 1,
                                  panel_prefix = "b. "
                                  )
```

```{r do l_f0 pairwise analysis, fig.height=2.5, fig.width = 3.01, results = "asis"}
printTidyPredictions(nuc_l_f0_mdl.trimmed)
getModelFixedFX(nuc_l_f0_mdl.trimmed,
                write = "5_nuc_output/nuc_l_f0")$slopes %>% 
  mutate(across(
          any_of(c("estimate", "conf.low", "conf.high")),
          ~ round(as.numeric(.), 1))) %>% tidyStatNumbers()


```

```{r l binomial plots, fig.height=5.5/2.54, fig.width=15/2.54}
# set terms to plot
terms = c("fin_phon [L%]",
          "nuc_new_word [T]",
          "gender [M]")

# create _t plot
l_t_binom_plot <- plot_model(nuc_l_t_mdl,
           show.values = T,
           show.intercept = T,
           terms = terms) +
  geom_hline(yintercept = 0, size = 0.5, color = "red") +
    ylab("estimate (ms)") +
    labs(
         caption = paste0 ("a. Nuclear l_t model binomial factor effects.")) +
    theme(
      plot.title = element_blank(),
      plot.caption = element_text(hjust = 0, size = 10),
      plot.caption.position = "plot",
      axis.title = element_text(size = 10)
    )

# create _f0 plot
l_f0_binom_plot <- plot_model(nuc_l_f0_mdl.trimmed,
           show.values = T,
           terms = terms) +
  geom_hline(yintercept = 0, size = 0.5, color = "red") +
    ylab("estimate (ST)") +
    labs(title = "",
         caption = paste0 ("b. Nuclear l_f0 model binomial factor effects.")) +
    theme(
      plot.title = element_blank(),
      plot.caption = element_text(hjust = 0, size = 10),
      plot.caption.position = "plot",
      axis.title = element_text(size = 10)
    )

# join the plots
grid.arrange(l_t_binom_plot, l_f0_binom_plot, ncol = 2)

# save the plots
ggsave("5_nuc_output/l_target_binomial.png",
        device = "png",
        arrangeGrob(l_t_binom_plot, l_f0_binom_plot, ncol = 2),
        width = 15.5,
        height = 5.5,
        units = "cm")
```
## H targets

### Analysis of H timing

```{r set h_t equation}
h_t_equation = formula(
  h_t ~
    foot_syls
  + pre_syls
  + fin_phon
  + nuc_new_word
  + gender
  + (1 | speaker)
  + (1 | nuc_str_syl)
)

optimizer = "optimx"
```

```{r run h_t models, fig.height=3, fig.width=9}
nuc_h_t_mdl = lmer(
  h_t_equation,
  nuc_data,
  control = lmerControl(
    optimizer = optimizer,
    calc.derivs = F,
    optCtrl = list(
      method = "nlminb",
      starttests = F,
      kkt = F
    )
  )
)

# model needs trimming
nuc_h_t_mdl = update(nuc_h_t_mdl,
                     data = nuc_data %>%
                       filter(abs(scale(resid(nuc_h_t_mdl))) <= 3))

anova <- summariseLME(nuc_h_t_mdl,
                      run_step = F,
                      write = "5_nuc_output/nuc_h_t_anova.csv")

adjustP_posthoc("5_nuc_output/",
                p.value,
                suffix_id = "_anova")

read_csv("5_nuc_output/nuc_h_t_anova.csv") %>%
  tidyStatNumbers() %>%
  formattable(caption = paste("ANOVA of model:",
                              getModelFormula(nuc_h_t_mdl)))
```


```{r h_t model summary, fig.height=2.5, fig.width=2.823, results="asis"}
nuc_h_t_mdl.tidy <- analyseModel(nuc_h_t_mdl,
                                write="5_nuc_output/nuc_h_t",
                                type = "pred",
                                y_lab = "time (ms)",
                                y_lim = c(150, 550),
                                breaks = seq(150, 550, 100),
                                plot_rounding = 0,
                                panel_prefix = "a. "
                                )


```

```{r do h_t pairwise analysis, fig.height=2.5, fig.width = 3.01, results = "asis"}
printTidyPredictions(nuc_h_t_mdl)
getModelFixedFX(nuc_h_t_mdl,
                write = "5_nuc_output/nuc_h_t")$slopes %>% 
  mutate(across(
          any_of(c("estimate", "conf.low", "conf.high")),
          ~ round(as.numeric(.),0))) %>% tidyStatNumbers()
```

### Analysis of H F0

```{r set h_f0 equation}
h_f0_equation = formula(
  h_f0 ~ 
    foot_syls
  + pre_syls
  + fin_phon
  + nuc_new_word
  + gender
  + (1 | speaker)
  + (1 | nuc_str_syl)
)

optimizer = "optimx"
```

```{r run h_f0 models, fig.height=3, fig.width=9}
nuc_h_f0_mdl = lmer(
  h_f0_equation,
  nuc_data,
  control = lmerControl(
    optimizer = optimizer,
    calc.derivs = F,
    optCtrl = list(
      method = "nlminb",
      starttests = F,
      kkt = F
    )
  )
)

# model needs trimming
nuc_h_f0_mdl.trimmed = update(nuc_h_f0_mdl,
                              data = nuc_data %>%
                                filter(abs(scale(resid(nuc_h_f0_mdl))) <= 4))

anova <- summariseLME(nuc_h_f0_mdl.trimmed,
                      run_step = F,
                      write = "5_nuc_output/nuc_h_f0_anova.csv")

adjustP_posthoc("5_nuc_output/",
                p.value,
                suffix_id = "_anova")

read_csv("5_nuc_output/nuc_h_f0_anova.csv") %>%
  tidyStatNumbers() %>%
  formattable(caption = paste("ANOVA of model:",
                              getModelFormula(nuc_h_f0_mdl.trimmed)))
```

```{r h_f0 model summary, fig.height=2.5, fig.width=2.823, results="asis"}
nuc_h_f0_mdl.tidy <- analyseModel(nuc_h_f0_mdl.trimmed,
                                write="5_nuc_output/nuc_h_f0",
                                type = "pred",
                                y_lab = "f0 (ST re median)",
                                y_lim = c(-1.5, 4.5),
                                breaks = seq(-1.5, 4.5, 1),
                                plot_rounding = 1,
                                panel_prefix = "b. "
                                )

```

```{r do h_f0 pairwise analysis, fig.height=2.5, fig.width = 3.01, results = "asis"}
printTidyPredictions(nuc_h_f0_mdl.trimmed)
getModelFixedFX(nuc_h_f0_mdl.trimmed,
                          write = "5_nuc_output/nuc_h_f0")$slopes %>% 
  mutate(across(
          any_of(c("estimate", "conf.low", "conf.high")),
          ~ round(as.numeric(.), 1))) %>% tidyStatNumbers()

```

```{r h binomial plots, fig.height=5.5/2.54, fig.width=15/2.54}
# set terms to plot
terms = c("fin_phon [L%]",
          "nuc_new_word [T]",
          "gender [M]")

# create _t plot
h_t_binom_plot <- plot_model(nuc_h_t_mdl,
           type = "est",
           transform = NULL,
           show.values = T,
           vline = "Red",
           terms = terms) +
  geom_hline(yintercept = 0, size = 0.5, color = "red") +
    ylab("estimate (ms)") +
    labs(
         caption = paste0 ("a. Nuclear h_t model binomial factor effects.")) +
    theme(
      plot.title = element_blank(),
      plot.caption = element_text(hjust = 0, size = 10),
      plot.caption.position = "plot",
      axis.title = element_text(size = 10)
    )

# create _f0 plot
h_f0_binom_plot <- plot_model(nuc_h_f0_mdl.trimmed,
           type = "est",
           transform = NULL,
           show.values = T,
           vline = "Red",
           terms = terms) +
  geom_hline(yintercept = 0, size = 0.5, color = "red") +
    ylab("estimate (ST)") +
    labs(title = "",
         caption = paste0 ("b. Nuclear h_f0 model binomial factor effects.")) +
    theme(
      plot.title = element_blank(),
      plot.caption = element_text(hjust = 0, size = 10),
      plot.caption.position = "plot",
      axis.title = element_text(size = 10)
    )

# join the plots
grid.arrange(h_t_binom_plot, h_f0_binom_plot, ncol = 2)

# save the plots
ggsave("5_nuc_output/h_target_binomial.png",
        device = "png",
        arrangeGrob(h_t_binom_plot, h_f0_binom_plot, ncol = 2),
        width = 15.5,
        height = 5.5,
        units = "cm")
```

## E targets
e_t is not important for evaluation. However, the timing has been modelled just for visualisation purposes.
```{r set e_t equation}
### Analysis of E timing
# E timing is not of much interest as it is just the last f0 time point in the IP... except in terms of seeing how h_t and e_t differ.

e_t_equation = formula(
  e_t ~ 
    foot_syls
  + pre_syls
  + fin_phon
  + nuc_new_word
  + gender
  + (1 | speaker)
  + (1 | nuc_str_syl)
)

optimizer = "optimx"
```

```{r run e_t models, fig.height=3, fig.width=9}
nuc_e_t_mdl = lmer(
  e_t_equation,
  nuc_data,
  control = lmerControl(
    optimizer = optimizer,
    calc.derivs = F,
    optCtrl = list(
      method = "nlminb",
      starttests = F,
      kkt = F
    )
  )
)

# model needs trimming
nuc_e_t_mdl.trimmed = update(nuc_e_t_mdl,
                             data = nuc_data %>%
                               filter(abs(scale(resid(nuc_e_t_mdl))) <= 4)
)

 anova <- summariseLME(nuc_e_t_mdl.trimmed,
             run_step = F,
             write = "5_nuc_output/nuc_e_t_anova.csv")
 
 adjustP_posthoc("5_nuc_output/",
                 p.value,
                 suffix_id = "_anova")
 
 read_csv("5_nuc_output/nuc_e_t_anova.csv") %>%
   tidyStatNumbers() %>%
   formattable(caption = paste("ANOVA of model:",
                               getModelFormula(nuc_e_t_mdl.trimmed)))
```

```{r e_t model summary, fig.height=2.5, fig.width=2.823, results="asis"}
nuc_e_t_mdl.tidy <- analyseModel(nuc_e_t_mdl.trimmed,
                                write="5_nuc_output/nuc_e_t",
                                type = "pred",
                                y_lab = "time (ms)",
                                y_lim = c(200, 650),
                                breaks = seq(200, 650, 100),
                                plot_rounding = 0,
                                panel_prefix = "a. "
                                )

```

```{r do e_t pairwise analysis, fig.height=2.5, fig.width = 3.01, results="asis"}
printTidyPredictions(nuc_e_t_mdl.trimmed)
getModelFixedFX(
  nuc_e_t_mdl.trimmed,
  write="5_nuc_output/nuc_e_t"
)$slopes %>% 
  mutate(across(
          any_of(c("estimate", "conf.low", "conf.high")),
          ~ round(as.numeric(.),0))) %>% tidyStatNumbers()
```

### Analysis of E F0

```{r set e_f0 equation}
e_f0_equation = formula(
  e_f0 ~ 
    foot_syls
  + pre_syls
  + fin_phon
  + nuc_new_word
  + gender
  + (1 | speaker)
  + (1 | nuc_str_syl)
)

optimizer = "optimx"
```

- model needs trimming (|sd| residuals > 3.5)
```{r run e_f0 models, fig.height=3, fig.width=9}
nuc_e_f0_mdl = lmer(
  e_f0_equation,
  nuc_data,
  control = lmerControl(
    optimizer = optimizer,
    calc.derivs = F,
    optCtrl = list(
      method = "nlminb",
      starttests = F,
      kkt = F
    )
  )
)

# model needs trimming
nuc_e_f0_mdl.trimmed = update(
  nuc_e_f0_mdl,
  data = nuc_data %>% filter(abs(scale(resid(nuc_e_f0_mdl))) <= 3.5))

 anova <- summariseLME(nuc_e_f0_mdl.trimmed,
             run_step = F,
             write = "5_nuc_output/nuc_e_f0_anova.csv")
 
 adjustP_posthoc("5_nuc_output/",
                 p.value,
                 suffix_id = "_anova")
 
 read_csv("5_nuc_output/nuc_e_f0_anova.csv") %>%
   tidyStatNumbers() %>%
   formattable(caption = paste("ANOVA of model:",
                               getModelFormula(nuc_e_f0_mdl.trimmed)))
```


```{r e_f0 model summary, fig.height=2.5, fig.width=2.823, results="asis"}
nuc_e_f0_mdl.tidy <- analyseModel(nuc_e_f0_mdl.trimmed,
                                write="5_nuc_output/nuc_e_f0",
                                type = "pred",
                                y_lab = "f0 (ST re median)",
                                y_lim = c(-7, 3.5),
                                breaks = seq(-7, 3.5, 2),
                                plot_rounding = 1,
                                panel_prefix = "letters"
                                )

```

```{r do e_f0 pairwise analysis, fig.height=2.5, fig.width = 3.01, results="asis"}
printTidyPredictions(nuc_e_f0_mdl.trimmed)
getModelFixedFX(nuc_e_f0_mdl.trimmed,
                write="5_nuc_output/nuc_e_f0")$slopes %>% 
  mutate(across(
          any_of(c("estimate", "conf.low", "conf.high")),
          ~ round(as.numeric(.), 1))) %>% tidyStatNumbers()
```

```{r e binomial plots, fig.height=5.5/2.54, fig.width=15/2.54}
# set terms to plot
terms = c("fin_phon [L%]",
          "nuc_new_word [T]",
          "gender [M]")

# create _t plot
e_t_binom_plot <- plot_model(nuc_e_t_mdl,
           type = "est",
           transform = NULL,
           show.values = T,
           vline = "Red",
           terms = terms) +
  geom_hline(yintercept = 0, size = 0.5, color = "red") +
    ylab("estimate (ms)") +
    labs(
         caption = paste0 ("a. Nuclear e_t model binomial factor effects.")) +
    theme(
      plot.title = element_blank(),
      plot.caption = element_text(hjust = 0, size = 10),
      plot.caption.position = "plot",
      axis.title = element_text(size = 10)
    )

# create _f0 plot
e_f0_binom_plot <- plot_model(nuc_e_f0_mdl.trimmed,
           type = "est",
           transform = NULL,
           show.values = T,
           vline = "Red",
           terms = terms) +
  geom_hline(yintercept = 0, size = 0.5, color = "red") +
    ylab("estimate (ST)") +
    labs(title = "",
         caption = paste0 ("b. Nuclear e_f0 model binomial factor effects.")) +
    theme(
      plot.title = element_blank(),
      plot.caption = element_text(hjust = 0, size = 10),
      plot.caption.position = "plot",
      axis.title = element_text(size = 10)
    )

# join the plots
grid.arrange(e_t_binom_plot, e_f0_binom_plot, ncol = 2)

# save the plots
ggsave("5_nuc_output/e_target_binomial.png",
        device = "png",
        arrangeGrob(e_t_binom_plot, e_f0_binom_plot, ncol = 2),
        width = 15.5,
        height = 5.5,
        units = "cm")
```
## Truncation and Compression Effects

### LH Excursion

```{r set f0_exc equation}

f0_exc_equation = formula(
 f0_exc ~ 
   foot_syls
  + (1 + foot_syls | speaker)
  #+ (1 | gender)
  + (1 | nuc_str_syl)
  + (1 | pre_syls)
  + (1 | fin_phon)
  #+ (1 | nuc_new_word)
)

optimizer = "optimx"
```

- model needs trimming (|sd| residuals > 2.5)
```{r run f0_exc models, fig.height=3, fig.width=9}
nuc_f0_exc_mdl = lmer(
  f0_exc_equation,
  nuc_data,
  control = lmerControl(
    optimizer = optimizer,
    calc.derivs = F,
    optCtrl = list(
      method = "nlminb",
      starttests = F,
      kkt = F
    )
  )
)

nuc_f0_exc_mdl <- optimizeModel(nuc_f0_exc_mdl)
anova <- summariseLME(nuc_f0_exc_mdl,
             run_step = F, print_summary = F)

# model needs trimming
nuc_f0_exc.trimmed <-
  nuc_data %>% filter(abs(scale(resid(nuc_f0_exc_mdl))) <= 2.5)

nuc_f0_exc_mdl.trimmed = update(
  nuc_f0_exc_mdl,
  data = nuc_data %>% filter(abs(scale(resid(nuc_f0_exc_mdl))) <= 2.5)
  )

anova <- summariseLME(nuc_f0_exc_mdl.trimmed,
             run_step = F,
             write = "5_nuc_output/nuc_f0_exc_anova.csv")

 adjustP_posthoc("5_nuc_output/",
                 p.value,
                 suffix_id = "_anova")
 
 read_csv("5_nuc_output/nuc_f0_exc_anova.csv") %>%
   tidyStatNumbers() %>%
   formattable(caption = paste("ANOVA of model:",
                               getModelFormula(nuc_f0_exc_mdl.trimmed)))
```
```{r plot f0_exc estimated effects, fig.height=2.5, fig.width=4.3}
a <- plot_model(
  nuc_f0_exc_mdl.trimmed,
  type = "est",
  transform = NULL,
  show.values = T
 ) + labs(
   caption =
     "b. Estimated effects on f0_exc."
   ) +
  theme(title=element_blank(),
                  plot.caption=element_text(hjust = 0, size = 10),
                  plot.caption.position= "plot") +
  geom_hline(yintercept = 0, size = 0.5, color = "red")
```

```{r f0_exc model summary, fig.height=2.5, fig.width=2.823, results="asis"}

nuc_f0_exc_mdl.tidy <- analyseModel(nuc_f0_exc_mdl.trimmed,
                                write="5_nuc_output/nuc_f0_exc",
                                type = "pred",
                                y_lab = "f0 (ST)",
                                panel_prefix = "a. ")

```

```{r do f0_exc pairwise analysis, fig.height=2.5, fig.width = 3.01, results="asis"}
printTidyPredictions(nuc_f0_exc_mdl.trimmed)
getModelFixedFX(
  nuc_f0_exc_mdl.trimmed,
  write="5_nuc_output/nuc_f0_exc"
)$slopes %>% 
  mutate(across(
          any_of(c("estimate", "conf.low", "conf.high")),
          ~ round(as.numeric(.), 1))) %>% tidyStatNumbers()


```


### LH Slope

Based on results from tonal target analysis, it looksl like compression and truncation may be in effect.
As compression leads to decrease in duration of rise (but not necessarily much descrease in excursion size), and as slope increases exponentially as duration decreases, slope has been logged here.

- model works once random intercepts of speech_rate and foot_dur removed
```{r set lh_slope equation}
nuc_data <- nuc_data %>% 
    mutate(log_lh_slope = log(lh_slope))

lh_slope_equation = formula(
  log_lh_slope ~
    foot_syls
  + (1 + foot_syls | speaker)
  #+ (1 | gender)
  + (1 | nuc_str_syl)
  + (1 | pre_syls)
  + (1 | fin_phon)
  #+ (1 | nuc_new_word)
)
optimizer = "optimx"
```

- model needs trimming (|sd| residuals > 3.5)
```{r run lh_slope models, fig.height=3, fig.width=9}
nuc_lh_slope_mdl = lmer(
  lh_slope_equation,
  nuc_data,
  control = lmerControl(
    optimizer = optimizer,
    calc.derivs = F,
    optCtrl = list(
      method = "nlminb",
      starttests = F,
      kkt = F
    )
  )
)
nuc_lh_slope_mdl <- optimizeModel(nuc_lh_slope_mdl, verbose = F, reject_nl = T)

# model needs trimming
nuc_lh_slope_mdl.trimmed = update(
  nuc_lh_slope_mdl,
  data = nuc_data %>% filter(abs(scale(resid(nuc_lh_slope_mdl))) <= 3.5))



nuc_lh_slope_mdl.trimmed <-
  optimizeModel(nuc_lh_slope_mdl.trimmed,
                verbose = F,
                reject_nl = T)

anova <- summariseLME(nuc_lh_slope_mdl.trimmed,
                      run_step = F,
                      write = "5_nuc_output/nuc_lh_slope_anova.csv")
 
 
 adjustP_posthoc("5_nuc_output/",
                 p.value,
                 suffix_id = "_anova")
 
 read_csv("5_nuc_output/nuc_lh_slope_anova.csv") %>%
   tidyStatNumbers() %>%
   formattable(caption = paste("ANOVA of model:",
                               getModelFormula(nuc_lh_slope_mdl.trimmed)))
```

```{r plot lh slope estimated effects, fig.height=2.5, fig.width=4.3}
b <- plot_model(
  nuc_lh_slope_mdl.trimmed,
  type = "est",
  transform = NULL,
  show.values = T
 ) + labs(
   caption =
     "b. Estimated effects on log_lh_slope."
   ) +
  theme(title=element_blank(),
                  plot.caption=element_text(hjust = 0, size = 10),
                  plot.caption.position= "plot") +
  geom_hline(yintercept = 0, size = 0.5, color = "red")
```

```{r lh_slope model summary, fig.height=2.5, fig.width=2.823, results="asis"}
nuc_lh_slope_mdl.tidy <- analyseModel(nuc_lh_slope_mdl.trimmed,
                                write="5_nuc_output/nuc_lh_slope",
                                type = "pred",
                                panel_prefix = "b. ",
                                y_lab = "log(ST/sec)",
                                plot_rounding = 2)
```

```{r do lh_slope pairwise analysis, results="asis"}
printTidyPredictions(nuc_lh_slope_mdl.trimmed)
getModelFixedFX(
  nuc_lh_slope_mdl.trimmed,
  write="5_nuc_output/nuc_lh_slope"
)$slopes %>% 
  mutate(across(
          any_of(c("estimate", "conf.low", "conf.high")),
          ~ round(as.numeric(.), 2))) %>% tidyStatNumbers()


```

```{r compare effects exc and log_slope effects, fig.height=2.5, fig.width=6}
grid.arrange(a, b, ncol = 2) 
ggsave("5_nuc_output/nuc_truncation_params.png",
       device = "png",
       arrangeGrob(a, b, ncol = 2),
       width = 15.7,
       height = 6.5,
       units = "cm")
```


## Additional model

### H time proprotional to duration of voicing


```{r  h_t_as_prop_of_voicing, fig.height=3, fig.width=9}
### Analysis of E timing
# E timing is not of much interest as it is just the last f0 time point in the IP

h_t_as_prop_of_voicing_equation = formula(
  h_t_as_prop_of_voicing ~ 
    foot_syls
 # + fin_phon
  + (1 + foot_syls + fin_phon | speaker)
  + (1 | gender)
  + (1 | nuc_str_syl)
)

optimizer = "optimx"

nuc_data <- nuc_data %>%
  mutate(h_t_as_prop_of_voicing = h_t  / e_t)

nuc_h_t_as_prop_of_voicing_mdl = lmer(
  h_t_as_prop_of_voicing_equation,
  nuc_data,
control = lmerControl(
  optimizer = "nloptwrap",
  optCtrl = list(algorithm = "NLOPT_LN_PRAXIS",
                 maxfun = 1e9,
                 maxeval = 1e7,
                 xtol_abs = 1e-9,
                 ftol_abs = 1e-9)))

nuc_h_t_as_prop_of_voicing_mdl <- nuc_h_t_as_prop_of_voicing_mdl %>% optimizeModel()
 anova <- summariseLME(nuc_h_t_as_prop_of_voicing_mdl,
             run_step = T,
             print_summary = F
             )
 
nuc_h_t_as_prop_of_voicing.trimmed <- nuc_data %>%
    filter(abs(scale(resid(nuc_h_t_as_prop_of_voicing_mdl))) <= 3)

# model needs trimming
nuc_h_t_as_prop_of_voicing_mdl.trimmed = update(
  nuc_h_t_as_prop_of_voicing_mdl,
  data = nuc_h_t_as_prop_of_voicing.trimmed,
  control = lmerControl(
    optimizer = optimizer,
    calc.derivs = F,
    optCtrl = list(
      method = "nlminb",
      starttests = F,
      kkt = F
    )
  )
)

 nuc_h_t_as_prop_of_voicing_mdl.trimmed <-
   nuc_h_t_as_prop_of_voicing_mdl.trimmed %>%
   optimizeModel() 

 anova <- summariseLME(nuc_h_t_as_prop_of_voicing_mdl.trimmed,
             run_step = F,
             write = "5_nuc_output/nuc_h_t_as_prop_of_voicing_anova.csv"
             )

 adjustP_posthoc("5_nuc_output/",
                 p.value,
                 suffix_id = "_anova")

 read_csv("5_nuc_output/nuc_h_t_as_prop_of_voicing_anova.csv") %>%
   tidyStatNumbers() %>%
   formattable(caption = paste("ANOVA of model:",
                               getModelFormula(nuc_h_t_as_prop_of_voicing_mdl.trimmed)))
```

```{r h_t_as_prop_of_voicing model analysis, fig.height=2.5, fig.width=2.823, results="asis"}
nuc_h_t_as_prop_of_voicing_mdl.tidy <- analyseModel(nuc_h_t_as_prop_of_voicing_mdl.trimmed,
                                write="5_nuc_output/nuc_h_t_as_prop_of_voicing",
                                type = "pred",
                                y_lab = "h_t / e_t",
                                y_lim = c(0.5, 1),
                                breaks = seq(0.5, 1, 0.1),
                                plot_rounding = 2,
                                panel_prefix = "a. "
                                )

printTidyPredictions(nuc_h_t_as_prop_of_voicing_mdl.trimmed)
getModelFixedFX(
  nuc_h_t_as_prop_of_voicing_mdl.trimmed,
  write="5_nuc_output/nuc_h_t_as_prop_of_voicing_mdl"
)$slopes %>% 
  mutate(across(
          any_of(c("estimate", "conf.low", "conf.high")),
          ~ round(as.numeric(.),2))) %>% tidyStatNumbers()
```

...
`l_t ~ acc_phon + gender + (1 | speaker) + (1 | ana_text) + (1 | nuc_pre_text)`
...
x <- summariseLME(pn_l_t_mdl.trimmed
             ,run_step = TRUE
             ,write = "6_4_output/pn_l_t_anova.csv"
             )
...
read_csv("6_4_output/pn_l_t_anova.csv") %>%
  tidyStatNumbers() %>%
...
```{r do l_t pairwise analysis, fig.height=2.5, fig.width=3.051181}
...
After initial full model, step() suggests: `l_f0 ~ ana_syls + pn_new_word + (1 | speaker)`
Model works with single random intercept but with all fixed factors.
...
x <- summariseLME(pn_l_f0_mdl.trimmed
             , run_step = TRUE
             , write = "6_4_output/pn_l_f0_anova.csv"
             )
...
read_csv("6_4_output/pn_l_f0_anova.csv") %>%
  tidyStatNumbers() %>%
formattable(caption = paste("ANOVA of model:",
                            getModelFormula(pn_l_f0_mdl.trimmed)))
...
```{r do l_f0 pairwise analysis, fig.height=2.5, fig.width=3.051181}
...
x <- summariseLME(
  pn_h_t_mdl.trimmed
  , run_step = TRUE
  , write = "6_4_output/pn_h_t_anova.csv"
  )
...
read_csv("6_4_output/pn_h_t_anova.csv") %>%
  tidyStatNumbers() %>%
formattable(caption = paste("ANOVA of model:",
...
```{r do h_t pairwise analysis, fig.height=2.5, fig.width=3.051181}
charts <- getModelFixedFX(
  pn_h_t_mdl.trimmed,
  write="pn_phonetic_models/pn_h_t"
)
...
## H timing original data 

This is to check the extent to which the previous results re ana_syls are an artefact of the larger dataset. This goes for other results too, but as ana_syls has such odd results, it was worth checking.

```{r orig model test, fig.height=3, fig.width=9}

pn_h_data_orig <- filter(pn_h_data,
                         stim %in% c("A0423", "A1422", "A2422", "A3422"))
                         
  
h_t_orig_equation = formula(
  h_t ~
    acc_phon
  + ana_syls
  + wrd_end_syl
...
# model needs trimming
pn_h_data_orig.trimmed <-
  pn_h_data_orig %>% filter(abs(scale(resid(pn_h_t_orig_mdl))) <= 2.5)

pn_h_t_orig_mdl.trimmed = lmer(
  h_t_orig_equation,
  pn_h_data_orig.trimmed,
  control = lmerControl(
    optimizer = optimizer,
...
x <- summariseLME(
  pn_h_t_orig_mdl.trimmed
  , run_step = TRUE
  , write = "6_4_output/pn_h_t_orig_anova.csv"
  , extra_text = "[using original model]"
  )
...
read_csv("6_4_output/pn_h_t_orig_anova.csv") %>%
  tidyStatNumbers() %>%
  formattable(caption = paste("ANOVA of model:",
                            getModelFormula(pn_h_t_orig_mdl.trimmed)))

```
```{r horig model summary, fig.height=2.5, fig.width=3.051181, results="asis"}
pn_h_t_orig_mdl.tidy <- analyseModel(
  pn_h_t_orig_mdl.trimmed
  , write="pn_phonetic_models/pn_h_t_orig"
  , type = "pred"
  , factor_matrix = TRUE
  )

pn_h_t_orig_mdl.tidy$table
...
first run: step() suggests: `h_f0 ~ ana_syls + foot_syls + (1 | speaker) + (1 | pn_str_syl)`
...
x <- summariseLME(
  pn_h_f0_mdl.trimmed
  , run_step = TRUE
  , write = "6_4_output/pn_h_f0_anova.csv"
  )
...
read_csv("6_4_output/pn_h_f0_anova.csv") %>%
  tidyStatNumbers() %>%
formattable(caption = paste("ANOVA of model:",
                            getModelFormula(pn_h_f0_mdl.trimmed)))
...
pn_h_f0_mdl.tidy <- analyseModel(pn_h_f0_mdl.trimmed,
...
```{r do h_f0 pairwise analysis, fig.height=2.5, fig.width=3.051181}
...
x <- summariseLME(pn_f0_exc_mdl.trimmed,
             run_step = TRUE,
             write = "6_4_output/pn_f0_exc_anova.csv")
...
read_csv("6_4_output/pn_f0_exc_anova.csv") %>%
  tidyStatNumbers() %>%
  formattable(caption = paste("ANOVA of model:",
...
```{r do f0_exc pairwise analysis, fig.height=2.5, fig.width=3.051181}
...
x <- summariseLME(pn_lh_slope_mdl.trimmed,
             ...
             write = "6_4_output/pn_lh_slope_anova.csv")

adjustP_posthoc("6_4_output/",
                p.value,
                suffix_id = "_anova")

read_csv("6_4_output/pn_lh_slope_anova.csv") %>%
  tidyStatNumbers() %>%
  formattable(caption = paste("ANOVA of model:",
...
```{r do lh_slope pairwise analysis, fig.height=2.5, fig.width=3.051181}

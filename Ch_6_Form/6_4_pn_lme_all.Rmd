...
tidyPrintPredictions(pn_h_f0_mdl)
charts$slopes
...
x <- summariseLME(pn_f0_exc_mdl, run_step = T, print_summary = F,
                  draw_resids = F)
...
pn_f0_exc_mdl <- update(pn_f0_exc_mdl, formula = f0_exc_equation)
#pn_f0_exc_mdl <- optLme4Mdl(pn_f0_exc_mdl, reject_nl = T, verbose = F)
x <- summariseLME(pn_f0_exc_mdl, run_step = F, print_summary = F)
# model needs trimming

pn_f0_exc.trimmed <-
  pn_tnc_data %>% filter(abs(scale(resid(pn_f0_exc_mdl))) <= 2.75)
...
a <- plot_model(
  ...
  show.values = T,
  vline = "Red",
 ) + labs(caption = "a. Estimated effects of foot_syls on F0_exc.") +
...
b <- plot_model(
  ...
  transform = NULL,
  show.values = T
 ) + labs(caption = "b. Estimated effects of foot_syls on log_lh_slope.") +
  theme(title=element_blank(),
                  plot.caption=element_text(hjust = 0, size = 10),
                  plot.caption.position= "plot") +
  geom_hline(yintercept = 0, size = 0.5, color = "red")
...
getModelFixedFX(pn_lh_slope_mdl.trimmed,
                write = "6_4_pn_all_output/pn_lh_slope")$slopes
tidyPrintPredictions(pn_lh_slope_mdl.trimmed)
charts$slopes

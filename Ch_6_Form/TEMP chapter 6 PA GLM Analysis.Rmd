...
for (i in 1:(numLevels)) {
    ...
    mutate(
      ...
      pairwise =
        if_else(
          ...
          "intercept",
          if_else(
            term %in% c("foot_syls2", "foot_syls3", "foot_syls4"),
            "N/A",
  ...
  ifelse(i > 1,
         y <-
           filter(y, term %notin% c(
             "foot_syls2", "foot_syls3", "foot_syls4"
           )),
         y <- y)
  
  keep = c("foot_syls2", "foot_syls3", "foot_syls4")
  for (j in i:4) {
  ...
  curLevels <- c(curLevels[2:numLevels], curLevels[1])
   ...
   pn_foot <- pn_foot %>% 
      mutate(foot_syls = factor(
        foot_syls,
        levels = curLevels
        )
        )
   
   #### FOR SOME REASON THIS ISN'T LOOPING TO MAKE EACH CONDITION THE INTERCEPT IN TURN FOR PAIRWISE COMPARISON
...
b0_GenderSR  <-
  filter(x, term %in% c("foot_syls2", "foot_syls3", "foot_syls4")) %>%
  select(-pairwise)

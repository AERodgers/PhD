...
PN_Type = "LH"
write.table(PN_anac, "data/PN_anac_LH.txt", sep = "\t")
...
#                }
    ...
    PN.anac.mdl.slope = lmer(
        slope  ~ ana_syls + gender + (1 + ana_syls | speaker),
        data = PN_anac,
        control = lmerControl(optimizer = "optimx",
                              calc.derivs = FALSE,
                              optCtrl = list(method = "nlminb",
    ...
    fileName = paste(folder, "PN.anac.mdl.L_t_", PN_Type, "_", PN_Type, "_", syls[1], "syl.txt", sep = "")

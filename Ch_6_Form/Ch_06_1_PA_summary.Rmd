...
summary(lm(`L*H` ~ mean_speech_rate, data = pn_by_speaker_ratio)) %>%
  ...
  tidyNumbers() %>% 
  formattable(caption = "linear regresion of L\\*H ~ speech rate")
...
summary(lm(`H*` ~ mean_speech_rate, data = pn_by_speaker_ratio)) %>%
  tidy() %>%
  rename(`t value` = statistic) %>% 
  tidyNumbers() %>% 
  formattable(caption = "linear regresion of H\\* ~ speech rate")
```

#### chi squared test for gender ~ acc_phon on adjusted data
```{r  fig.width = 4.5, fig.height = 4}
pn_by_speaker_ratio_uncount %>%
  ungroup() %>% 
  select(gender, acc_phon) %>% 
  chisq_test(acc_phon ~ gender) %>% 
  rename(`ChiSq` = statistic) %>% 
  tidyNumbers("p_value", ) %>% 
  formattable(caption = "Chi-Squared test of acc_phon ~ gender") 
```

### correlation between gender mean and speech rate on adjusted data
```{r  fig.width = 4.5, fig.height = 4}
lm(mean_speech_rate ~ gender, data = pn_by_speaker_ratio) %>% 
  tidy() %>%
  rename(`t value` = statistic) %>% 
  tidyNumbers() %>% 
  formattable(caption = "linear regresion of mean_speech_rate ~ gender")
```

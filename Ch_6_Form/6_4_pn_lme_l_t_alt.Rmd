...
```{r late test, fig.height=2.5, fig.width=5.051181}
...
#pn_l_t_mdl_late <- optLme4Mdl(pn_l_t_mdl_late, reject_nl = T, verbose = F)
...
anova <- summariseLME(
  ...
  run_step = F,
  draw_resids = F,
  write = "6_4_pn_all_output/pn_l_t_late_anova.csv"
  )

plot_model(pn_l_t_mdl_late)
```


```{r late test part 2, fig.height=2.5, fig.width=3.051181}
adjustP_posthoc("6_4_pn_all_output/",
                p.value,
                suffix_id = "_anova")

read_csv("6_4_pn_all_output/pn_l_t_late_anova.csv") %>%
  tidyStatNumbers() %>%
...
pn_l_t_mdl_late.tidy <- analyseModel(
  pn_l_t_mdl_late,
  write="6_4_pn_all_output/pn_l_t_late",
  type = NULL,
  factor_matrix = TRUE,
  y_lab = "time (ms)",
  y_lim = c(-70, 200)

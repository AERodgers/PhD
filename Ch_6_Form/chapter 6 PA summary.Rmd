...
balancedData <- function(data_set, treatment_col, response_col,
  ...
  balanced <- balanced %>% 
    mutate(across(pa_columns,  ~  (.x / acc_count * num_reps))) %>%  # Convert tokens to ratios of tokens per speaker per target.
    group_by(!!treatment_col) %>% 
  ...
  balanced <- balanced %>%  
    ...
    mutate(across(pa_columns,  ~  round(.x / speakers * num_speakers))) %>%    # (Is there a more efficient way to do this?)
    select(-speakers)
...
## 1.1.2 Pre-nuclear Pitch Accents

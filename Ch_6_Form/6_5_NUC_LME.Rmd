...
diff_optims <- allFit(nuc_l_t_mdl, maxfun = 1e5, parallel = 'multicore', ncpus = ncores)
print(diff_optims)
...
plot_model(nuc_l_f0_mdl, type = "re")[2:4]
...
-  model needs trimming (residual sd > 3)
...
nuc_h_t_data.trimmed <-
  nuc_data %>% filter(abs(scale(resid(nuc_h_t_mdl))) <= 3)
...
- singularity likely false positive as allFit() results are good. 
...
# model needs trimming
nuc_h_f0.trimmed <-
  nuc_data %>% filter(abs(scale(resid(nuc_h_f0_mdl))) <= 3.5)
...
allFit(nuc_h_f0_mdl.trimmed) %>% print() 
```
...
- However, complete model words, so retaining it.
...
- Only intercepts only model avoided convergence and singularity issues
- initial step() suggested: `e_f0 ~ foot_syls + fin_phon + speech_rate + (1 | speaker) + (1 | nuc_pre_text)`
- removing foot_dur only way to remove singularity
...
- BUT full model works so retaining it.
### LH Excursion
...
initial step() suggests: `log_lh_slope ~ foot_syls + speech_rate + (1 | speaker) + (1 | nuc_pre_text)`
- model works so keeping it.

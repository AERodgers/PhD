...
ggplot(data = pn_wrd_data_sum,
  ...
  theme(legend.position = "none") +
  labs(caption = "PN peak timing for L*H for H-corpus sentence pairs (adjusted data)")


```

```{r plot with mean data grouped, fig.width = 6.3, fig.height = 2.5}
#ggplot(data = pn_wrd_data %>%  mutate(pn_wrds = factor(pn_wrds, levels = pn_phr)),
#       mapping = aes(x = mean_h_t, fill = pairing)) +
#  geom_density(stat="density") +
#  facet_wrap(vars(pn_wrds)) +
...
```{r plot with mean data 3, fig.width = 4.3, fig.height = 3}
...
ggplot(data = pn_wrd_data_sum %>% filter(pn_wrds %in% c("elaine was a",
  ...
  mapping = aes(x = mean_h_t, fill = pn_wrds)) +
  geom_density(stat="density", alpha = 0.75) +
  scale_fill_manual(values=c(brewer.pal(8, "Dark2")[1],
                             brewer.pal(8, "Set2")[1]),
                    name=NULL) +
  theme(legend.position = "right") +
  labs(caption = "PN peak timing for L*H for 'Elaine was a' and 'Elaina's a' (adjusted data)")

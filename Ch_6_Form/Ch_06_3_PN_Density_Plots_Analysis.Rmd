...
```{r sketchy hartigan, results = "asis"}
require("diptest")
...
valeries_is = pn_wrd_data %>%
  filter(pn_wrds %in% pn_phr[2],
         speaker %in% c("F6", "F15", "F16", "M5", "M10"),
         acc_phon == "L*H")  %>% select(h_grand_mean_t)


elaine_was = pn_wrd_data %>%
    filter(
      pn_wrds %in% pn_phr_cut[1],
      speaker %in% c("F6", "F12", "F15", "F16", "M5", "M10"),
      acc_phon == "L*H"
...
dt <- dip.test(vals_is$h_grand_mean_t)
...
dt <- dip.test(valeries_is$h_grand_mean_t)
D.value <- c(D.value, as.numeric(dt[1]))
p.value  <- c(p.value, as.numeric(dt[2]))
observations <- c(observations, as.numeric(dt[3]))


dt <- dip.test(elaine_was$h_grand_mean_t)
D.value <- c(D.value, as.numeric(dt[1]))
p.value  <- c(p.value, as.numeric(dt[2]))
observations <- c(observations, as.numeric(dt[3]))
...
dip_test_summary <- tibble(`target phrase` = pn_phr,
                           observations,
                           D.value =  round(D.value, 2),
                           p.value = round(p.value, 3),
                           `p.adj (BH)` = round(p.adjust(p.value, "BH"), 3))
dip_test_summary %>%
  sigCodesTidy(`p.adj (BH)`, TRUE) %>% 
kable(caption = "Hartigans' dip test for unimodality / multimodality. (Alternative hypothesis: non-unimodal, i.e., at least bimodal.)")

```

```{r sketchy hartigan sets, results = "asis", , fig.width = 3.5, fig.height = 2.2}
require("diptest")

set_1 <- rbind(vals_is, lallys_is)
set_2 <- rbind(lallys_is_in, valeries_is)
set_3 <- rbind(elaine_was, elainas_a)

ggplot(
  set_3,
  mapping = aes(x = h_grand_mean_t)
) +
  geom_dotplot(alpha = 0.5) +
  geom_density(stat = "density", fill = "grey", alpha = 0.5) + ylim(0, 0.01) +
  xlim(0, 550) +
  ylim(0, 0.01) +
  xlab("grand mean syllable-normalised time (ms)") +
  ylab("density")
...
dt <- dip.test(set_1$h_grand_mean_t)
D.value <- c(D.value, as.numeric(dt[1]))
p.value <- c(p.value, as.numeric(dt[2]))
observations <- c(observations, as.numeric(dt[3]))

dt <- dip.test(set_2$h_grand_mean_t)
D.value <- c(D.value, as.numeric(dt[1]))
p.value  <- c(p.value, as.numeric(dt[2]))
observations <- c(observations, as.numeric(dt[3]))


dt <- dip.test(set_3$h_grand_mean_t)
D.value <- c(D.value, as.numeric(dt[1]))
p.value  <- c(p.value, as.numeric(dt[2]))
observations <- c(observations, as.numeric(dt[3]))

dip_test_summary <- tibble(set = sets,
                           observations,
                           D.value =  round(D.value, 2),
                           p.value = round(p.value, 3),
                           `p.adj (BH)` = round(p.adjust(p.value, "BH"), 3))

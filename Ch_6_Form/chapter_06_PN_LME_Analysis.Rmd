...
cat("Tibble factors for pn_l_type\n")
for (i in colnames(pn_l_type))
{
  if (is.factor(pn_l_type[[eval(i)]]))
  {
    cat("\n", i, "  \tlevels = ", levels(pn_l_type[[i]]))
  }
  else
  {
    cat("\n", i)
...
cat("\n\nTibble factors for pn_h_type\n")
for (i in colnames(pn_h_type))
{
  if (is.factor(pn_h_type[[eval(i)]]))
  {
    cat("\n", i, "  \tlevels = ", levels(pn_h_type[[i]]))
  }
    else
  {
    cat("\n", i)
...
- model needs trimming (residuals +/- 5 s.d.)
...
pn_h_t.trimmed <-
  pn_h_type %>% filter(abs(scale(resid(pn_h_t_mdl))) <= 5)
...
# model needs trimming
pn_l_t.trimmed <-
  pn_l_type %>% filter(abs(scale(resid(pn_l_t_mdl))) <= 3.5)
...
# model needs trimming
pn_l_f0.trimmed <-
  pn_l_type %>% filter(abs(scale(resid(pn_l_f0_mdl))) <= 4)

...
#### `lh_mean_f0 ~  mode + fin_phon + gender + (1 + mode | speaker) + (1 | prompt)`
```{r run initial model,  fig.height=3, fig.width=9, tidy = TRUE}

# run model
lh_mean_f0_model <-
    lmer(
        lh_mean_f0 ~  mode + fin_phon + gender +
...
lh_mean_f0_model.tidy <- analyseModel(lh_mean_f0_model,
                                write_r2="../output/acc_phon_lh_mean_f0"
                                )
...
charts <- getModelFixedFX(
  my_equation = lh_mean_f0 ~  mode + fin_phon + gender +
    (1 + mode | speaker) + (1 | prompt),

  my_data = m_corpus,
  write="../output/acc_phon_lh_mean_f0"
)
charts$intercepts
charts$slopes

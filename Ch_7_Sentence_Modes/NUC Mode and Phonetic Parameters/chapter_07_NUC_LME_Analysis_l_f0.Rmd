...
Running new model again but trimming rows with |residual| > 3.4 (as per Baayen 2008: 279-282).

NB a value of 3.4 was chosen as this removes the minimum number of outliers without leading to singularity errors while at the same time providing a more reasonably distribution or residuals.

```{r fig.height=3, fig.width=9, warning = FALSE, message = FALSE, tidy = TRUE}
m_corpus_trimmed <- m_corpus %>% filter(abs(scale(resid(temp_mdl))) < 3.4)

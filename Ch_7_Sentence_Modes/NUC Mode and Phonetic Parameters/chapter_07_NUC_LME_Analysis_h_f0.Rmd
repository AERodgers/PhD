...
* Distribution of residuals is fine.
...
```
* Reduced model caused singularity error.
* Reverted to original BUT this led to singularity issues with pairwise comparison.
* Removing gender as fixed effect, since random effects of speaker should also account for the variance contributed by gender (i.e. gender is a fixed parameter for each speaker).
...
* Singularity issue resolved.

NB: Gender will have to be removed from the other f0 parameter tests to ensure that parity across intercepts.  

...
m_corpus_H_or_not <- m_corpus %>%
  # treat HL% as for purpose of % vs H%
  mutate(fin_phon_0H =  str_replace(fin_phon, "HL\\%", "H%")) %>% 
  mutate(fin_phon_0H = factor(fin_phon_0H=="H%", level = c(F, T))) %>% 
...
anova(fin_phon_model_H_or_not, fin_phon_model_H_or_not_null, test="Chisq") %>%
  formattable(

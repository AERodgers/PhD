---
title: 'Chapter 07: Analysis of Function: Sentence Modes and Phonology, IP and nuclear PA phonology (No Register Tier)'
output:
...
m_corpus %>%
  ...
  write_csv("nuc_by_speaker_raw_no_reg.csv") %>%
...
pa_by_speaker_adj <- pa_adj %>%
  ...
  select("speaker", "H*", "L*H", ">H*")  %>% 
  write_csv("nucs_by_speaker_adj_no_reg.csv")
...
pa_by_mode_adj <- pa_adj %>%
  ...
  select("mode", "H*", "L*H", ">H*")  %>% 
  arrange(mode) %>% 
  write_csv("modes_nuc_balanced_no_reg.csv")
...
pa_by_gender_adj <- pa_adj %>%
  ...
  arrange(mode) %>% 
  write_csv("modes_nuc_gender_adj_no_reg.csv")
...
```{r warning=FALSE}
pa_by_speaker_adj %>%
  write_csv("nuc_by_speaker_adj_no_reg.csv")  %>%
  rename_all(list(~str_replace_all(., "([\\*\\[\\^\\>])", "\\\\\\1"))) %>%  
  adorn_totals(where = "row") %>% 
  formattable(
...
Even without the Chi^2 test, it is clear that there is little relationship between the pitch accent and mode when the register tier is discounted. However, looking at the raw data for the phrase-level phonology, it looks like L% boundaries are more commonly associated with questions than declaratives.

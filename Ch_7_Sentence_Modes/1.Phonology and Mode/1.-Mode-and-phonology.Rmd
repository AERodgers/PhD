...
installMissingPackages(
  c("tidyverse", "formattable", "janitor", "knitr", "RColorBrewer", "kableExtra")
)
...
## Utterance level intonational phonology (raw data)

```{r list utterence level phonology, warning=FALSE}
...
nuc_only <- m_corpus %>%
  mutate(one_star = str_count(phr_phon, "\\*")==1) %>% 
  filter(one_star == TRUE) %>%
  group_by(mode) %>%
  summarise(phon_count = n(), .groups="rowwise") %>%
  arrange(desc(phon_count)) %>%
  pivot_wider(names_from = mode,
...
nuc_only %>% 
  ...
  mutate("info" =c("count", "percentage"), .before=1) %>%
  kable(
  caption = "Utterance level intonational phonology by mode for with high register")%>% 
  kable_styling(full_width = FALSE, position="left")
  ...
  formattable(
    ...
    list(
      area(col = 3:6) ~ proportion_bar(color = c(
        rep(brewer.pal(4, "Pastel2")[3], 16),
        rep(brewer.pal(4, "Pastel2")[2], 16),
        rep(brewer.pal(4, "Pastel2")[1], 16),
        rep(brewer.pal(4, "Pastel2")[4], 16)
        )),
...
```{r utterance phonology by mode, exlcuding MWH, warning=FALSE}
...
m_corpus %>%
  ...
  formattable(
    ...
    list(area(col = 3:5) ~ proportion_bar(color = c(
        rep(brewer.pal(4, "Pastel2")[3], 12),
        rep(brewer.pal(4, "Pastel2")[1], 12),
        rep(brewer.pal(4, "Pastel2")[4], 12)
        )),
      total = proportion_bar(color = "#aaaaaa")
    )
    )
...
```{r chi squared of raw data}
m_corpus %>% tabyl(mode, acc_phon) %>% chisq.test()
```
...
m_corpus %>%
    ...
    tabyl(mode, acc_phon) %>% chisq.test()
...
Firstly a tibble is created summarising the proportion of pitch accents per mode per speaker. This is done in three initial stages:
    ...
    3. 3.   Convert the proportion per stimulus per speaker to a proportion per stimulus per mode.
...
```{r} pa_by_mode_adj_list %\>% tabyl(mode, acc_phon) %\>% chisq.test()```

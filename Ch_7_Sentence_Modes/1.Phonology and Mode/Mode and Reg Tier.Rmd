...
#### Raw Data
```{r summarise nuc contour , warning=FALSE, results="asis"}
...
```{r summarise nuc contour by speaker (raw data), warning=FALSE}
m_corpus %>%
...
```{r model charts and figures, fig.width = 5, fig.height = 2.5, results="asis"}
cat(
...
```{r extra chart Charts, fig.width = 4.2, fig.height = 2.5}
ggpredict(h_reg_model, terms=c("mode", "gender")) %>%
  plot() +
  labs(caption = "", title = "") +
  ylim(0,1) +
  ylab("prob. of high reg in nuc.") +
   geom_text(aes(
...
fin_phon_model_null = 
  cur_model <- bglmer(
  `L% boundary` ~ 1 + (1 | speaker) + (1 | prompt),
  data = m_corpus_boundaries,
  family = binomial(link = "logit"),
  control = glmerControl(
    optimizer = "optimx",
...
charts <- getModelFixedFX(
  my_equation = `L% boundary` ~ mode + gender + (1 | speaker) + (1 | prompt),
...
```{r extra chart L boundary, fig.width = 4.2, fig.height = 2.5, results="asis"}
ggpredict(fin_phon_model, terms=c("mode")) %>%
  plot() +
  labs(caption = "", title = "") +
  ylim(0,1) +
  ylab("prob. of L%") +
...
ggpredict(fin_phon_model, terms=c("mode")) %>%
  ...
  mutate(across(2:last_col(),
         ...
         across(2:4,
         ~ percent(., 1))) %>% 
  formattable(caption="probability of L%")
```

...
m_corpus <- get_m_corpus("../data/m_corpus.csv")  %>%
  mutate(nuc_reg = factor(if_else(str_detect(acc_phon, "\\^"),"H","L"),
                          levels = c("L", "H"))) %>% 
...
# NB: GLMER cannot estimate likelihood of H:L for MDC and MWH since there are no instances of H for either level.
# Therefore, the odds ratio for L is 1:0 against and for H it is 0:1 for,
# making the probability 0 but the CIs stretch from 0 to infinity...
# 
# At least, that's what I think is happening!
# 
# Apparently it is called "complete separation"
# 
# Workaround: add dummy speaker with one instance of each possible outcome for 
# each level of mode.
# 
m_c_avoid_separation <- m_corpus %>%
  select(speaker, nuc_reg, mode) %>%
  ...
  mutate(
    speaker = factor(speaker, levels = unique(speaker)),
    nuc_reg = factor(nuc_reg, level = c("L", "H")),
    mode = factor(mode, c("MDC", "MWH", "MYN", "MDQ"))
  )
  

nuc_reg_model <- glmer(
  nuc_reg ~ mode + (1 | speaker),
  data = m_c_avoid_separation,
...
charts <- getModelFixedFX(
  my_equation = nuc_reg ~ mode + (1 | speaker),
  my_data = m_c_avoid_separation,
  write="Mode_PA_GLMM",
  is_GLM=TRUE,
...
m_corpus_boundaries <- m_corpus %>% 
  mutate(fin_phon = factor(str_replace_all(fin_phon, "\\[|\\]|\\^", ""),
                           levels=c("%", "L%")))
                           
fin_phon_model <- glmer(
  fin_phon ~ mode + (1 | speaker),
...
x <- printTidyModel(fin_phon_model,
                    write_r2="Mode_PA_GLMM",
                    is_GLM = TRUE)
x$r2
x$table
...
charts <- getModelFixedFX(
  my_equation = fin_phon ~ mode + (1 | speaker),

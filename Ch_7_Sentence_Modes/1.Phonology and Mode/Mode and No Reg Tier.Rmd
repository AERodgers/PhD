...
#### __GLMM analysis for % v H% tier__
```{r GLMM Test of % and H% re mode}


fin_phon_model_0H <- bglmer(
  `% or H% (no reg tier)` ~ mode + gender + (1 | speaker) + (1 | prompt),
  data = m_corpus_0H,
  family = binomial(link = "logit"),
  fixef.prior = normal()
)

fin_phon_model_0H_null <-
  bglmer(
    `% or H% (no reg tier)` ~ 1 + (1 | speaker) + (1 | prompt),
    data = m_corpus_0H,
    family = binomial(link = "logit")
  )
...
fin_phon_model_0L <- glmer(
  `% or L% (no reg tier)` ~ mode + gender + (1 | speaker) + (1 | prompt),
  data = m_corpus_0L,
  family = binomial(link = "logit")
)

fin_phon_model_0L_null <- glmer(
  `% or L% (no reg tier)` ~ 1 + (1 | speaker) + (1 | prompt),
  data = m_corpus_0L,
  family = binomial(link = "logit")
)

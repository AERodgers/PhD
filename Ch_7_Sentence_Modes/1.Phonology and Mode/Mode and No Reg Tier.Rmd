...
fin_phon_model_0H <- bglmer(`% or H% (no reg tier)` ~ mode + (1 | speaker),
                           data = m_corpus_0H,
...
tidy_summary <- analyseModel(fin_phon_model_0H, "Mode_fin_phon_no_Reg_0H", is_GLM = TRUE)

tidy_summary$r2
tidy_summary$table

charts <- getModelFixedFX(
  my_equation = `% or H% (no reg tier)` ~ mode + (1 | speaker),
  my_data = m_corpus_0H,
  is_separation = TRUE,
...
fin_phon_model_0L <- glmer(`% or L% (no reg tier)` ~ mode + (1 | speaker),
                           data = m_corpus_0L,
                           family = binomial(link = "logit"))
...
tidy_summary <- analyseModel(fin_phon_model_0L, "Mode_fin_phon_no_Reg_0L", is_GLM = TRUE)
tidy_summary$r2
tidy_summary$table

charts <- getModelFixedFX(
  my_equation = `% or L% (no reg tier)` ~ mode + (1 | speaker),
  my_data = m_corpus_0L,

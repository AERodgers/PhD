...
installMissingPackages(
  c("tidyverse", "formattable", "janitor", "knitr", "RColorBrewer", "lme4")
)
```
...
#### GLMM tests of fin\_phon as a fn of mode.

A preliminary test returned a genderM with a probability  of 1.015 (CIs=0.118--8.721, p=0.9894), indicating that gender likely has no effect, Therefore, it was removed from the final model.


* __Temporary workaround for complete separation issue__
* Workaround: create dummy speaker with one entry per level of each alternative outcome.

```{r Make binomial subsets of corpus}
m_corpus_0H <- m_corpus %>%
  # treat HL% as for purpose of % vs H%
  mutate(fin_phon =  str_replace(fin_phon, "HL\\%", "H%")) %>% 
  filter(fin_phon %in% c("%", "H%")) %>% 
  # Add anti-separation workaround: add dummy speaker with one instance of each
  # alternate values for each level of mode.
  select(speaker, fin_phon, mode) %>%
  mutate(speaker = factor(speaker, levels = c(levels(speaker), "dummy"))) %>%
  add_row(
    speaker = rep("dummy", 8),
    fin_phon = c(rep("H%", 4), rep("%", 4)),
    mode = rep(c("MDC", "MWH", "MYN", "MDQ"),2)
  ) %>%
  # adding rows appears to remove factors from columns. Need to redo them!
  mutate(
    speaker = factor(speaker, levels = unique(speaker)),
    fin_phon = factor(fin_phon, level = c("%", "H%")),
    mode = factor(mode, c("MDC", "MWH", "MYN", "MDQ")))



  
m_corpus_0L <- m_corpus %>%
  # treat HL% as for purpose of % vs L%
  mutate(fin_phon, str_replace(fin_phon, "HL\\%", "L%")) %>% 
  filter(fin_phon %in% c("%", "L%")) %>% 
  mutate(fin_phon = factor(fin_phon, levels = c("%", "L%")))
```

#### __`%|H% ~ mode + (1 | speaker)`__
```{r GLMM Test of % and H% re mode}
my_formula = formula(fin_phon ~ mode + (1 | speaker))

fin_phon_model_0H <- glmer(my_formula,
                           data = m_corpus_0H,
                           family = binomial(link = "logit"))

fin_phon_model_0H_null <- glmer(fin_phon ~ 1 + (1 | speaker),
                                data = m_corpus_0H,
                                family = binomial(link = "logit"))

summary(fin_phon_model_0H)
```


```{r GLMM of 0 and H re mode Figures and Charts, fig.width = 4.2, fig.height = 2.5, results="asis"}
cat(
  "isSingular(fin_phon_model_0H, tol=1e-5) -->",
  isSingular(fin_phon_model_0H, tol = 1e-5),
  "\n"
)

anova(fin_phon_model_0H, fin_phon_model_0H_null, test="Chisq") %>%
  formattable(
    caption="ANOVA of model and null model, assuming no register tier."
    )

x <- printTidyModel(fin_phon_model_0H, "Mode_fin_phon_no_Reg_0H", is_GLM = TRUE)

x$r2
x$table

charts <- getModelFixedFX(
  my_equation = my_formula,
  my_data = m_corpus_0H,
  write="Mode_fin_phon_no_Reg_0H",
  is_GLM=TRUE,
  extra_text = ", zero and high, assuming no register tier.",
  report="intercepts"
)
charts$intercepts
charts$slopes
...
#### `%|L% ~ mode + (1 | speaker)`
```{r GLMM Test of 0 and L re mode}
my_formula = formula(fin_phon ~ mode + (1 | speaker))
                                
fin_phon_model_0L <- glmer(my_formula,
                           data = m_corpus_0L,
                           family = binomial(link = "logit"))

fin_phon_model_0L_null <- glmer(fin_phon ~ 1 + (1 | speaker),
                                data = m_corpus_0L,
                                family = binomial(link = "logit"))

summary(fin_phon_model_0L)
```


```{r GLMM of 0 and L re mode Figures and Charts, fig.width = 4.2, fig.height = 2.5, results="asis"}
cat(
  "isSingular(fin_phon_model_0L, tol=1e-5) -->",
  isSingular(fin_phon_model_0L, tol = 1e-5),
  "\n"
)

anova(fin_phon_model_0L, fin_phon_model_0L_null, test="Chisq") %>%
  formattable(
    caption="ANOVA of model and null model, assuming no register tier."
    )

x <- printTidyModel(fin_phon_model_0L, "Mode_fin_phon_no_Reg_0L", is_GLM = TRUE)
x$r2
x$table
...
charts <- getModelFixedFX(
  my_equation = my_formula,
  my_data = m_corpus_0L,
  write="Mode_fin_phon_no_Reg_0L",
  is_GLM=TRUE,
  extra_text = ", zero and low, assuming no register tier.",
  report="intercepts"
)

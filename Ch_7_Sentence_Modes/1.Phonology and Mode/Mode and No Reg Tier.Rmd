...
m_corpus_0H <- m_corpus %>%
  ...
  mutate(fin_phon_0H =  str_replace(fin_phon, "HL\\%", "H%")) %>% 
  filter(fin_phon_0H %in% c("%", "H%")) %>% 
  mutate(fin_phon_0H = factor(fin_phon_0H, level = c("%", "H%"))) %>% 
  rename("% or H% (no reg tier)"= fin_phon_0H)

m_corpus_0L <- m_corpus %>%
  # treat HL% as for purpose of % vs L%
  mutate(fin_phon_0L = str_replace(fin_phon, "HL\\%", "L%")) %>% 
  filter(fin_phon_0L %in% c("%", "L%")) %>% 
  mutate(fin_phon_0L = factor(fin_phon_0L, levels = c("%", "L%"))) %>% 
  rename("% or L% (no reg tier)"= fin_phon_0L)
```
...
fin_phon_model_0H <- bglmer(`% or H% (no reg tier)` ~ mode + gender + (1 | speaker),
                           data = m_corpus_0H,
                           family = binomial(link = "logit"),
                           fixef.prior = normal())

fin_phon_model_0H_null <- bglmer(`% or H% (no reg tier)` ~ 1 + (1 | speaker),
                                 data = m_corpus_0H,
                                 family = binomial(link = "logit"))
...
tidy_summary <- analyseModel(fin_phon_model_0H,
...
charts <- getModelFixedFX(
  my_equation = `% or H% (no reg tier)` ~ mode + gender + (1 | speaker),
  my_data = m_corpus_0H,
  is_separation = TRUE,
  write="Mode_fin_phon_no_Reg_0H",
  is_GLM=TRUE,
  extra_text = ", non-register tier analysis."
)
charts$intercepts
charts$slopes
```
...
fin_phon_model_0L <- glmer(`% or L% (no reg tier)` ~ mode + gender + (1 | speaker),
                           data = m_corpus_0L,
                           family = binomial(link = "logit"))

fin_phon_model_0L_null <- glmer(`% or L% (no reg tier)` ~ 1 + (1 | speaker),
                                data = m_corpus_0L,
                                family = binomial(link = "logit"))
...
tidy_summary <- analyseModel(fin_phon_model_0L,
                             "Mode_fin_phon_no_Reg_0L",
                             is_GLM = TRUE,
                             type = "pred")
tidy_summary$r2
tidy_summary$table
...
charts <- getModelFixedFX(
  my_equation = `% or L% (no reg tier)` ~ mode + gender + (1 | speaker),
  my_data = m_corpus_0L,

...
m_corpus <- get_m_corpus("../data/m_corpus.csv")  %>%
  ...
  mutate(
    ...
    `nuclear PA only` = str_count(phr_phon, "\\*")==1,
    `nuclear PA only` = factor(`nuclear PA only`, levels=unique(`nuclear PA only`)),
    `high register in utterance` =  factor(str_detect(phr_phon, "\\^"), levels = c(F, T)),
    `high reg. or nuc. PA only` = factor((`high register in utterance` == T | `nuclear PA only` ==T), levels = c(F, T))
    ) %>%
...
```{r Show `high register in utterance` utterance level phonology summary, warning=FALSE}
uses_h_reg <- m_corpus %>%
  filter(`high register in utterance` == T) %>%
  group_by(mode) %>%
  summarise(phon_count = n(), .groups="rowwise") %>%
  arrange(desc(phon_count)) %>%
  pivot_wider(names_from = mode,
...
uses_h_reg_or <- m_corpus %>%
  filter(`high register in utterance` == T | `nuclear PA only` == T) %>%
  group_by(mode) %>%
  summarise(phon_count = n(), .groups="rowwise") %>%
  arrange(desc(phon_count)) %>%
  pivot_wider(names_from = mode,
...
```{r h_reg_model}
h_reg_model <- bglmer(
  `high register in utterance` ~ mode + gender + (1 | speaker),
  data = m_corpus,
...
h_reg_model_null = 
  cur_model <- bglmer(
  `high register in utterance` ~ 1 + (1 | speaker),
  data = m_corpus,
  family = binomial(link = "logit"),
  # change optimizer to avoid convergence errors
  control = glmerControl(
...
h_reg_charts <- getModelFixedFX(
  my_equation = `high register in utterance` ~ mode + gender +(1 | speaker),
  my_data = m_corpus,
...
one_pa_model <- bglmer(
  `nuclear PA only` ~ mode + gender +(1 | speaker),
  data = m_corpus,
  family = binomial(link = "logit"),
  # change optimizer to avoid convergence errors
  control = glmerControl(
...
one_pa_charts <- getModelFixedFX(
  my_equation = `nuclear PA only` ~ mode + gender + (1 | speaker),
  my_data = m_corpus,
...
h_reg_or_model <- bglmer(
  `high reg. or nuc. PA only` ~ mode + gender +(1 | speaker),
  data = m_corpus,
  family = binomial(link = "logit"),
  # change optimizer to avoid convergence errors
  control = glmerControl(
...
h_reg_or_null = 
  cur_model <- bglmer(
   `high reg. or nuc. PA only` ~ 1 + (1 | speaker),
  data = m_corpus,
  family = binomial(link = "logit"),
  # change optimizer to avoid convergence errors
  control = glmerControl(
...
h_reg_or_charts <- getModelFixedFX(
  my_equation = `high reg. or nuc. PA only` ~ mode + gender +(1 | speaker),
  my_data = m_corpus,

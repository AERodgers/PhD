...
```{r h_reg_model}
h_reg_model <- bglmer(
  `high register in utterance` ~ mode + (1 | speaker),
  data = m_corpus,
...
h_reg_charts <- getModelFixedFX(
  my_equation = `high register in utterance` ~ mode + (1 | speaker),
  my_data = m_corpus,
...
one_pa_model <- bglmer(
  `nuclear PA only` ~ mode + (1 | speaker),
  data = m_corpus,
  family = binomial(link = "logit"),
  # change optimizer to avoid convergence errors
  control = glmerControl(
...
one_pa_charts <- getModelFixedFX(
  my_equation = `nuclear PA only` ~ mode + (1 | speaker),
  my_data = m_corpus,
...
h_reg_or_model <- bglmer(
  `high reg. or nuc. PA only` ~ mode + (1 | speaker),
  data = m_corpus,
  family = binomial(link = "logit"),
  # change optimizer to avoid convergence errors
  control = glmerControl(
...
h_reg_or_charts <- getModelFixedFX(
  my_equation = `high reg. or nuc. PA only` ~ mode + (1 | speaker),
  my_data = m_corpus,

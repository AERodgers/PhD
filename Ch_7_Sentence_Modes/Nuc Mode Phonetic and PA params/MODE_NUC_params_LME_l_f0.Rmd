...
cur_equation <-
  "l_f0 ~ mode + acc_phon + fin_phon + gender + (1 + mode | speaker)"
```

### LME model analysis using model: `r cur_equation`

```{r check model,  fig.height=3, fig.width=9, message = FALSE, tidy = TRUE}
temp_mdl <- my_lme(cur_equation, m_corpus, show_results=FALSE, run_step=FALSE)

# pre-empt outlier
m_corpus.trimmed <- m_corpus %>% filter(abs(scale(resid(temp_mdl))) <= 3.5)
temp_mdl <- my_lme(cur_equation, m_corpus.trimmed, run_step=FALSE)
...
printTidyModel(
  ...
  bf_adj = 7,
  exclude_terms = "",
  write_r2 = "../output/LME_PHonetic_PA_l_f0_r2.csv"
)
...
for (i in 1:(num_levels)) {
  ...
  bonferroniAdjust(14, exclude_terms = c("GenderM", "fin_phonL%")) %>%
  rename(`p.adj. (bf=14)` = p.adj) %>% 
    mutate(
...
filter(all_models_tidy, pairwise == "intercept") %>%
  ...
  sigCodesTidy(`p.adj. (bf=14)`) %>% mutate(
    ...
    `p.adj. (bf=14)` = if_else(
      `p.adj. (bf=14)` < 0.001,
      as.character(scientific(`p.adj. (bf=14)`, digits=2)),
      as.character(round(`p.adj. (bf=14)`, 4))
    )
  ) %>% 
  write_csv("../output/LME_PHonetic_PA_l_f0_b0.csv")  %>%
  formattable(list(`p.adj. (bf=14)` = formatter("span", style = p_color)),
...
filter(all_models_tidy, pairwise %notin% c("intercept", "N/A")) %>%
  ...
  rename(`p.adj. (bf=14)` = `p.adj. (bf=14)`) %>%
  write_csv("../output/LME_PHonetic_PA_l_f0_b1.csv") %>%
  formattable(list(`p.adj. (bf=14)` = formatter("span", style = p_color)),

...
printTidyModel(
  ...
  bf_adj = 14,
  exclude_terms = "",
  write_r2 = "../output/LME_PHonetic_PA_h_f0_r2.csv"
)
...
for (i in 1:(num_levels)) {
  ...
  cur_modeh_f0idy <- tidy(cur_model) %>%
  ...
  # Bind tidy model to...
  cur_modeh_f0idy <- cbind(cur_modeh_f0idy, ci_Wald) %>%
    # re-order columns
    select(
      "term",
      "estimate",
  ...
  cur_modeh_f0idy <- filter(cur_modeh_f0idy, term %in% keep)
  
  # add remaining pairwise comparisons to main tibble.
  all_models_tidy <- bind_rows(all_models_tidy, cur_modeh_f0idy)
...
filter(all_models_tidy, pairwise == "intercept") %>%
  ...
  write_csv("../output/LME_PHonetic_PA_h_f0_b0.csv")  %>%
...
filter(all_models_tidy, pairwise %notin% c("intercept", "N/A")) %>%
  ...
  write_csv("../output/LME_PHonetic_PA_h_f0_b1.csv") %>%
  formattable(list(`p.adj. (bf=7)` = formatter("span", style = p_color)),

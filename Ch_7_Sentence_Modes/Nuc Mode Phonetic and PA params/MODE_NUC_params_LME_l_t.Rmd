...
cur_equation <-
  "l_t ~ mode + acc_phon + fin_phon + gender + (1 + mode | speaker) + (1 | prompt)"
```
...
temp_mdl <- my_lme(cur_equation, m_corpus.trimmed, run_step=TRUE)
...
printTidyModel(
  ...
  bf_adj = 7,
  exclude_terms = "",
  write_r2 = "../output/LME_PHonetic_PA_l_t_r2.csv"
)
...
filter(all_models_tidy, pairwise == "intercept") %>%
  ...
  write_csv("../output/LME_PHonetic_PA_l_t_b0.csv")  %>%
...
filter(all_models_tidy, pairwise %notin% c("intercept", "N/A")) %>%
  ...
  write_csv("../output/LME_PHonetic_PA_l_t_b1.csv") %>%
  formattable(list(`p.adj. (bf=7)` = formatter("span", style = p_color)),

---
title: "Chapter 7 - LME analysis of Mode and Nuclear Contours: h_t"
...
cur_equation <-
  "h_t ~ mode + fin_phon + gender + (1 + mode | speaker) + (1 | prompt) "
...
* Model found from step regression: 
    * `h_t ~ mode + fin_phon + gender + (1 | prompt) + (1 | speaker)`
* Prompt significant.
* fin_phon significant.
* Singularity issues.
...
do.call(rbind, r2_nakagawa(temp_mdl))[,1] %>% 
  write.csv("../output/LME_h_t_r2.csv")
...
filter(all_models_tidy, pairwise == "intercept") %>%
  ...
  write_csv("../output/LME_h_t_b0.csv")  %>%
...
filter(all_models_tidy, pairwise %notin% c("intercept", "N/A")) %>%
  ...
  write_csv("../output/LME_h_t_b1.csv") %>%
  formattable(list(`p.adj. (bf=7)` = formatter("span", style = p_color)),

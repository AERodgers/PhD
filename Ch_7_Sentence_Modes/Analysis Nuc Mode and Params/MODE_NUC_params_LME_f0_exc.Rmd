---
title: "Chapter 7 - LME analysis of Mode and Nuclear Contours: f0_exc"
...
```{r loadFns, message=FALSE, warning=FALSE, include=FALSE}
...
NB: Gender caused singularity issues for another f0 parameter test (l_f0). However, it is not excluded here, since excursion size intercepts will not be used to plot stylized PAs contours.
...
-   Model found from step regression:
    -   `f0_exc ~ mode + fin_phon + (1 + mode | speaker)`
-   Prompt is non-significant.
-   Gender is non-significant.
-   Singularity problem!

### LME model analysis using reduced models
...
-   Singularity issue disappears with reduced model.
-   Gender eliminated
-   Model found: `f0_exc ~ mode + fin_phon + (1 + mode | speaker)`
...
# set current equation
cur_equation <-
  "f0_exc ~ mode + fin_phon + (1 + mode | speaker) + (1 | prompt)"
...
-   Singularity issue returns.
-   Gender eliminated
-   Best final model, most likely: `f0_exc ~ mode + fin_phon + (1 + mode | speaker)`

#### Testing model without gender or prompt
...
kable(
    ...
    caption = paste("Conditional and marginal R^2^ of model", cur_equation),
    digits = 2,
    align = "l"
  )

do.call(rbind, r2_nakagawa(temp_mdl))[,1] %>% 
  write.csv("../output/LME_f0_exc_r2.csv")
...
filter(all_models_tidy, pairwise == "intercept") %>%
  ...
  write_csv("../output/LME_f0_exc_b0.csv")  %>%
  mutate(p.value = scientific(p.value, digits = 2)) %>%
...
filter(all_models_tidy, pairwise %notin% c("intercept", "N/A")) %>%
  ...
  write_csv("../output/LME_f0_exc_b1.csv") %>%
  mutate(p.value = scientific(p.value, digits = 2)) %>% 
  formattable(list(`p.adj. (bf=7)` = formatter("span", style = p_color)),
              caption = paste("b1 for", cur_equation, sep = " "))
```

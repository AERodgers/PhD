---
title: "Chapter 7 - LME analysis of Mode and Nuclear Contours: lh_slope"
...
NB: Gender caused singularity issues for another f0 parameter test (l_f0). However, it is not excluded here, since slope and f0 tonal targets are not directly comparable.
...
-   Model found from step regression:
    -   `lh_slope ~ mode + fin_phon + (1 + mode | speaker) + (1 | prompt)`
-   Gender is non-significant.
-   Unlike the l_f0 and h_f0 models, prompt is significant here. This is because slope is a composite parameter f0 / time parameter.
-   Distribution of residuals is slightly leptokurtic.

### LME model analysis using reduced model and removing outliers (\|residual\|\>=2.5)

Running it again but trimming rows with \|residual\|\>=2.5 (as per Baayen 2008: 279-282).
...
cat("\n\nCheck_singularity(temp_mdl, tolerance = 1e-05 -->", check_singularity(temp_mdl, tolerance = 1e-05, "\n"))

```

-   Distribution of still residuals is leptokurtic.

### LME model analysis using reduced model and removing outliers (\|residual\|\>2.5)

Running new model again but trimming rows with \|residual\| \> 2.5 (as per Baayen 2008: 279-282).

```{r fig.height=3, fig.width=9, warning = FALSE, message = FALSE, tidy = TRUE}
m_corpus.trimmed <- m_corpus %>% filter(abs(scale(resid(temp_mdl))) < 3)
...
cat("\n\nCheck_singularity(temp_mdl.trimmed, tolerance = 1e-05 -->", check_singularity(temp_mdl.trimmed, tolerance = 1e-05, "\n"))
cat("\n")


removed <- m_corpus %>% filter(abs(scale(resid(temp_mdl))) >= 3) 
removed %>% 
  select(c(speaker, gender, mode, prompt, phr_phon, lh_slope, lh_slope_z)) %>% 
...
do.call(rbind, r2_nakagawa(temp_mdl))[,1] %>% 
  write.csv("../output/LME_lh_slope_r2.csv")
...
filter(all_models_tidy, pairwise == "intercept") %>%
  ...
  write_csv("../output/LME_lh_slope_b0.csv")  %>%
...
filter(all_models_tidy, pairwise %notin% c("intercept", "N/A")) %>%
  ...
  write_csv("../output/LME_lh_slope_b1.csv") %>%
  formattable(list(`p.adj. (bf=7)` = formatter("span", style = p_color)),

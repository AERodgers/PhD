...
```{r load functions and packages, message=FALSE, warning=FALSE, include=FALSE}
...
cur_equation <-
  "h_t ~ acc_phon + fin_phon + gender + (1 + acc_phon | speaker) + (1 | prompt) "
```
...
-   No outliers.
-   Model borderline singular, and step suggests an all intercepts model.
-   As per Mode analysis, changing this to intercepts only random effects.

```{r set and test intercepts only model, fig.height=3, fig.width=9, warning = FALSE, message = FALSE, tidy = TRUE}
# set current equation
cur_equation <-
  "h_t ~ acc_phon + fin_phon + gender + (1 | speaker) + (1 | prompt) "
...
```{r get final output, fig.height=3, fig.width=9, warning = FALSE, message = FALSE, tidy = TRUE}

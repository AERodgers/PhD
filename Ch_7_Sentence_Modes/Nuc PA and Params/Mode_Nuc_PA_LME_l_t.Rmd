---
title: "Chapter 7 - LME analysis of Nuclear Contours: l_t"
...
cur_equation <-
  "l_t ~ acc_phon + fin_phon + gender + (1 + acc_phon | speaker)+ (1 | prompt) "
```
...
NB a value of 3.5 was chosen to remove extreme outlier.
...
removed %>% 
  select(c(speaker, gender, mode, prompt, acc_phon, fin_phon, l_t)) %>% 
...
cbind(tidy_model, ci_Wald) %>%
  ...
  formattable(
    ...
    list(`p.val. adj. ()` = formatter("span", style = p_color))
  ) %>%
  mutate(
    p.value = if_else(
      p.value < 0.001,
...
do.call(rbind, r2_nakagawa(temp_mdl))[,1] %>% 
  write.csv("../output/LME_PA_l_t_r2.csv")
...
filter(all_models_tidy, pairwise == "intercept") %>%
  ...
  write_csv("../output/LME_PA_l_t_b0.csv") %>%
...
filter(all_models_tidy, pairwise %notin% c("intercept", "N/A")) %>%
  ...
  write_csv("../output/LME_PA_l_t_b1.csv")  %>%
  mutate(

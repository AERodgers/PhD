---
title: "Chapter 7 - LME analysis of Nuclear Contours: h_f0"
...
cur_equation <- "h_f0 ~ acc_phon + fin_phon + gender + (1 + acc_phon | speaker)"
...
* Distribution of residuals is okay.
...
do.call(rbind, r2_nakagawa(temp_mdl))[,1] %>% 
  write.csv("../output/LME_PA_h_f0_r2.csv")
...
filter(all_models_tidy, pairwise == "intercept") %>%
  ...
  write_csv("../output/LME_PA_h_f0_b0.csv") %>%
...
filter(all_models_tidy, pairwise %notin% c("intercept", "N/A")) %>%
  ...
  write_csv("../output/LME_PA_h_f0_b1.csv")  %>%
  mutate(

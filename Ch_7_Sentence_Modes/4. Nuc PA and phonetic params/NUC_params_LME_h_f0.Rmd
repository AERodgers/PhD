---
title: "Chapter 7 - LME analysis of h_f0 ~ Nuclear pitch accents: "
...
*    Initial model threw up singularity issue with `(1|prompt)`, which `step()` suggests is non-significant. Removing it, as in other cases
*    step() suggests that `fin_phon` is non-significant. Keeping it as it is in other models of h_f0.

```{r run initial model,  fig.height=3, fig.width=9, tidy = TRUE}

# run model
h_f0_model <-
  lmer(h_f0 ~ acc_phon + fin_phon + gender + (1 + acc_phon | speaker),
       data = m_corpus,
        control = lmerControl(
          optimizer = "optimx",
          calc.derivs = FALSE,

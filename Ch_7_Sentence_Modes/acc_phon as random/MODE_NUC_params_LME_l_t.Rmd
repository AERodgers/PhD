...
# run model
l_t_model <-
  lmer(l_t ~  mode + acc_phon + gender + (1 + acc_phon| speaker),
       data = m_corpus_rises)
m_corpus_rises.trimmed <- m_corpus_rises %>% filter(abs(scale(resid(l_t_model))) < 3.5)
...
summarise_lme(l_t_model_2, run_step=TRUE)

```

```{r make tidy model, fig.height=3, fig.width=9, warning = FALSE, message = FALSE, tidy = TRUE}

l_t_model.tidy <- printTidyModel(l_t_model_2,
                                bf_adj = 8,
...
charts <- getLMEFixedFX(
  my_equation = l_t ~  mode + acc_phon + gender + (1 + acc_phon| speaker),

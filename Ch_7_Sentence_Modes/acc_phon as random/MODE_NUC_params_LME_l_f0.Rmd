...
Straight to best model

```{r run run intercepts only and reduced model without prompt,  fig.height=3, fig.width=9, message = FALSE, tidy = TRUE}
...
l_f0_model <-
  lmer(l_f0 ~ mode + acc_phon + fin_phon + gender + (1 + acc_phon| speaker),
    data = m_corpus_rises)

m_corpus_rises.trimmed <- m_corpus_rises %>% filter(abs(scale(resid(l_f0_model))) < 2.5)

l_f0_model_reduced <-
  lmer(l_f0 ~ mode + acc_phon + fin_phon + gender + (1 + acc_phon | speaker),
    data = m_corpus_rises.trimmed)

summarise_lme(l_f0_model_reduced , run_step=TRUE)

```
* step() suggests acc_phon is non-significant as a fixed factor.

```{r make tidy model, fig.height=3, fig.width=9, warning = FALSE, message = FALSE, tidy = TRUE}

l_f0_model.tidy <- printTidyModel(l_f0_model_reduced,
                                bf_adj = 8,
                                exclude_terms = c("fin_phonL%", "genderM"),
                                write_r2 = "LME_PHonetic_PA_l_f0_RA"
                                )
...
charts <- getLMEFixedFX(
  my_equation = l_f0 ~ mode + acc_phon + fin_phon + gender + (1 + acc_phon| speaker),
  my_data = m_corpus_rises.trimmed,

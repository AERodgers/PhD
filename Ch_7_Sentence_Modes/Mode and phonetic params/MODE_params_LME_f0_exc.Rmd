...
ive the sparsity of \^[L\*]H (n=6), H\* (n=2) and \>H \*(n=5), and considering that they are limited to a single speaker, I wanted to exclude then from the analysis.
...
The initial model `f0_exc ~ mode + fin_phon + gender + (1 + mode | speaker) + (1|prompt)`---as with the models for other phonetic parameters---has singularity issues, making it unreliable. Therefore, the by-speaker random effect `(1 | prampt` was removed, leading to:
...
charts <- getLMEFixedFX(
  my_equation = f0_exc ~ mode + fin_phon + (1 + mode | speaker),
  my_data = m_corpus,

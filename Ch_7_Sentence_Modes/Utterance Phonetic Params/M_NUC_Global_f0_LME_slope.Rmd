...
#### Trimming corpus using SD of 2 and use equation from step() equation
```{r}
# set current equation
cur_equation <-
...
```{r fig.height=3, fig.width=9, warning = FALSE, message = FALSE, tidy = TRUE}
m_corpus.trimmed <- m_corpus %>% filter(abs(scale(resid(temp_mdl))) < 2.1)
...
removed <- m_corpus %>% filter(abs(scale(resid(temp_mdl))) >= 2.1) 
removed %>% 
...
```{r fig.height=3, fig.width=9, warning = FALSE, message = FALSE, tidy = TRUE}
tidy_model <- tidy(temp_mdl.trimmed) %>%
  filter(term != "sd__(Intercept)") %>%
  bonferroniAdjust(2, exclude_terms = "genderM") %>%
...
```{r fig.height=3, fig.width=9, warning = FALSE, message = FALSE, tidy = TRUE}
kable(
    r2_nakagawa(temp_mdl.trimmed),
    caption = "Conditional and marginal R^2^ of model",
    digits = 2,
    align = "l"
  )

do.call(rbind, r2_nakagawa(temp_mdl.trimmed))[,1] %>% 

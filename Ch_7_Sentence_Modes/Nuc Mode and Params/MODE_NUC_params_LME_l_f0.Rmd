---
title: "Chapter 7 - LME analysis of Mode and Nuclear Contours: l_f0"
...
cur_equation <-
  "l_f0  ~ mode + fin_phon + gender + (1 + mode | speaker) + (1 | prompt)"
...
NB: Gender was originally included as a fixed effect; however, because this caused singularity issues for other f0 parameter tests (l_f0), it has been excluded. Note that f0 gender effects will be compensated for though the random effect of speaker.
...
* Model found from step regression: 
    * `l_f0 ~ mode + fin_phon + (1 + mode | speaker)`
* Prompt is non-significant.
* Distribution of residuals is leptokurtic.
* Retaining original model for parity across all NUC LME phhonetic parameter analyses.
...
removed %>% 
  select(c(speaker, gender, mode, prompt, phr_phon, l_f0, l_f0_z)) %>% 
...
tidy_model <- tidy(temp_mdl) %>%
  ...
  bonferroniAdjust(7, c("fin_phonL%", "genderM"))  %>%
...
do.call(rbind, r2_nakagawa(temp_mdl))[,1] %>% 
  write.csv("../output/LME_l_f0_r2.csv")
...
filter(all_models_tidy, pairwise == "intercept") %>%
  ...
  write_csv("../output/LME_l_f0_b0.csv")  %>%
...
filter(all_models_tidy, pairwise %notin% c("intercept", "N/A")) %>%
  ...
  write_csv("../output/LME_l_f0_b1.csv") %>%
  formattable(list(`p.adj. (bf=7)` = formatter("span", style = p_color)),

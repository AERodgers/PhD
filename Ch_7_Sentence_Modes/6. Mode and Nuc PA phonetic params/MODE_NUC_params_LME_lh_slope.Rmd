...
lh_slope_model <-
    lmer(
        lh_slope ~ acc_phon * mode + fin_phon + prompt +
...
summarise_lme(lh_slope_model, run_step=TRUE)
...
```{r run reduced step() only model,  fig.height=3, fig.width=9, ,  message = FALSE, tidy = TRUE}

lh_slope_model <-
    lmer(lh_slope ~ acc_phon + mode + prompt + (1 + acc_phon + mode | speaker) + acc_phon:mode,
        data = m_corpus,
        control = lmerControl(
            optimizer = "optimx",
            calc.derivs = FALSE,
...
-   Trying intercepts only mode (and pre-empting outlier)
-   Not changing model, but trimming outliers.

```{r trimmed data on intercepts only model tested, fig.height=3, fig.width=9, ,  message = FALSE, tidy = TRUE}
m_corpus.trimmed <- m_corpus %>% filter(abs(scale(resid(lh_slope_model))) <= 3)

lh_slope_model.trimmed <-
    lmer(lh_slope ~ acc_phon * mode + fin_phon + prompt +
      (1 | speaker) + (1 | gender),
...
summarise_lme(lh_slope_model.trimmed, run_step = FALSE,
...
lh_slope_model.tidy <- printTidyModel(lh_slope_model.trimmed,
                                bf_adj = 0,
                                write_r2="../output/Mode_PA_lh_slope"
                                )
lh_slope_model.tidy$table %>% 
  filter(term %in% c("fin_phonL%","genderM")) %>% 
  write_csv("../output_2_level_slope/Mode_PA_lh_slope_2_level.csv") %>% print() 
...
charts <- getModelFixedFX(
  my_equation = lh_slope ~ acc_phon * mode + fin_phon + prompt +
      (1 | speaker) + (1 | gender),

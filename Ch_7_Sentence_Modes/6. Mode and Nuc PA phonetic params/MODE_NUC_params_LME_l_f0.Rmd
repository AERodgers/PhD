...
l_f0_model <-
  lmer(
    l_f0 ~ acc_phon * mode + fin_phon + prompt +
...
```{r run step() models,  fig.height=3, fig.width=9, ,  message = TRUE, warning = TRUE, tidy = TRUE}
l_f0_model <-
  lmer(
    l_f0 ~ acc_phon * mode + fin_phon + (1 + acc_phon + mode | speaker),
    data = m_corpus,
    control = lmerControl(
      optimizer = "optimx",
      calc.derivs = FALSE,
...
summarise_lme(l_f0_model, run_step = FALSE)
```
-    Optimization issue persist.
-    changing to intercepts only model. 

```{r run intercepts only model, fig.height=3, fig.width=9, ,  message = TRUE, warning = TRUE, tidy = TRUE}
l_f0_model <-
  lmer(
    l_f0 ~ acc_phon * mode + fin_phon + prompt +
      (1 | speaker) + (1 | gender),
...
m_corpus.trim <- m_corpus %>% filter(abs(scale(resid(l_f0_model))) <= 3)
...
l_f0_model.tidy$table %>% 
  write_csv("../output_2_level_slope/Mode_PA_l_f0_2_level.csv") %>% print() 
l_f0_model.tidy$r2
...
charts <- getModelFixedFX(
  my_equation = l_f0 ~ acc_phon * mode + fin_phon + prompt +
      (1 | speaker) + (1 | gender),
  my_data = m_corpus.trim,

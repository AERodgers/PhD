...
# run model
l_t_model <-
  lmer(l_t ~ acc_phon * mode + fin_phon + prompt +
...
```{r run step() model,  fig.height=3, fig.width=9, ,  tidy = TRUE}
l_t_model <- lmer(l_t ~ acc_phon * mode + prompt +
                    (1 + acc_phon + mode | speaker) + (1 | gender),
       data = m_corpus,
        control = lmerControl(
          optimizer = "optimx",
...
-   Convergence and singularity issues.
-   Trying intercetps only model.
-   Removing interaction, as it has no effect on R2 and p.values are <0.09
...
l_t_model.trimmed <- lmer(l_t ~ acc_phon + mode + prompt + fin_phon +
                    (1 | speaker) + (1 | gender),
...
-    Retaining fin_phon, even though probably not significant.
...
l_t_model.tidy$table %>% 
  filter(term %in% c("fin_phonL%","genderM")) %>% 
  write_csv("../output_2_level_slope/Mode_PA_l_t_2_level.csv") %>% print() 
l_t_model.tidy$r2
...
charts <- getModelFixedFX(
  my_equation =  l_t ~ acc_phon + mode + prompt + fin_phon +
                    (1 | speaker) + (1 | gender),

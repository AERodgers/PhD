...
The initial model `f0_exc ~ mode + acc_phon + mode:acc_phon + fin_phon + gender + (1 + acc_phon | speaker)`---as with the models for other phonetic parameters---has singularity issues, making it unreliable. Therefore, the by-speaker random effects were simplified to `(1 + acc_phon | speaker)`, leading to:

-   `f0_exc ~ mode + acc_phon + mode:acc_phon + fin_phon + gender + (1 + acc_phon | speaker)`
...
-   `acc_phon:mode` does not appear appear significant as a fixed effect.
-   Using model found from step()

#### `f0_exc ~ mode + acc_phon + fin_phon + (1 + acc_phon | speaker)`

```{r run intercepts only model,  fig.height=3, fig.width=9, message = FALSE, tidy = TRUE}

f0_exc_model_reduced <-
    lmer(f0_exc ~ mode + acc_phon + fin_phon + (1 + acc_phon | speaker),
        data = m_corpus,
        control = lmerControl(
            optimizer = "optimx",
            calc.derivs = FALSE,
...
f0_exc_model.tidy <- printTidyModel(f0_exc_model_reduced,
                                bf_adj = 8,
                                exclude_terms = c("fin_phonL%", "genderM"),
                                write_r2 = "LME_PHonetic_PA_f0_exc"
                                )
...
charts <- getModelFixedFX(
  my_equation = f0_exc ~ mode + acc_phon + fin_phon + (1 + acc_phon | speaker),
  my_data = m_corpus,
  exclude_terms = c("gender", "fin_phon"),
  bf_adj = 8,
  write = "LME_Phonetic_PA_f0_exc"
)
charts$intercepts
charts$pairwise

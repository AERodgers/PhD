...
The initial model is: `h_f0 ~ mode + acc_phon + mode:acc_phon + fin_phon + (1 | gender) + (1 + acc_phon | speaker)`

```{r run initial model,  fig.height=3, fig.width=9, tidy = TRUE}

# run model
h_f0_model <-
  lmer(h_f0 ~ mode + acc_phon + mode:acc_phon + fin_phon +
         (1 | gender) + (1 + mode + acc_phon | speaker),
       data = m_corpus,
        control = lmerControl(
          optimizer = "optimx",
          calc.derivs = FALSE,
...
-   Neither (1 | gender) nor (1 | prompt) appear significant as a random effects.

#### `h_f0 ~ mode + acc_phon + fin_phon + (1 + mode + acc_phon | speaker)`
...
h_f0_model_reduced <-
    lmer(h_f0 ~ mode + acc_phon + fin_phon + (1 + mode + acc_phon | speaker),
        data = m_corpus,
        control = lmerControl(
            optimizer = "optimx",
            calc.derivs = FALSE,
...
summarise_lme(h_f0_model_reduced , run_step = FALSE)
...
charts <- getModelFixedFX(
  my_equation = h_f0 ~ mode + acc_phon + fin_phon + (1 + mode + acc_phon | speaker),
  my_data = m_corpus,

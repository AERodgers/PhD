...
cur_equation <- "log(abs(lh_slope)) ~ acc_phon + (1 | speaker) + (1 | prompt)"

```

Using the model `log(abs(lh_slope)) ~ acc_phon + (1 | speaker) + (1 | prompt)` we have to assume that the slopes will share the same polarity. Othersise, the model will not be (quasi)linear.

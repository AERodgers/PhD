---
title: "Chapter 7 - LME analysis of Nuclear Contours: lh_slope"
...
cur_equation <- "lh_slope ~ acc_phon + fin_phon + gender + (1 | speaker)"
```
...
* QQ plot suggests nonlinear correlation.
* change lh_slope to log(lh_slope)

```{r}
# set current equation
cur_equation <- "log(lh_slope) ~ acc_phon + fin_phon + gender + (1 | speaker)"
```
...
* Plots indicate that log(lh_slope) generates a better model fit.
...
do.call(rbind, r2_nakagawa(temp_mdl))[,1] %>% 
  write.csv("../output/LME_PA_lh_slope_r2.csv")
...
filter(all_models_tidy, pairwise == "intercept") %>%
  ...
  write_csv("../output/LME_PA_lh_slope_b0.csv") %>%
...
filter(all_models_tidy, pairwise %notin% c("intercept", "N/A")) %>%
  ...
  write_csv("../output/LME_PA_lh_slope_b1.csv")  %>%
  mutate(

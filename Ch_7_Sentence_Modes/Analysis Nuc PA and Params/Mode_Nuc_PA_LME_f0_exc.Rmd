...
cbind(tidy_model, ci_Wald) %>%
  ...
  formattable(
    ...
    list(`p.val. adj. (bf=7)` = formatter("span", style = p_color))
  ) %>%
  mutate(
    p.value = if_else(
      p.value < 0.001,
    ...
    `p.val. adj. (bf=7)` = if_else(
      `p.val. adj. (bf=7)` < 0.001,
      as.character(scientific(`p.val. adj. (bf=7)`, digits=2)),
      as.character(round(`p.val. adj. (bf=7)`, 4))
    )
  )
```
...
filter(all_models_tidy, pairwise == "intercept") %>%
  ...
  mutate(intercept = str_replace_all(intercept,
    ...
    formattable(list(`p.val. adj. (bf=7)` = formatter("span", style = p_color)),
...
filter(all_models_tidy, pairwise %notin% c("intercept", "N/A")) %>%
  ...
  sigCodesTidy(`p.val. adj. (bf=7)`) %>% mutate(
    `p.val. adj. (bf=7)` = if_else(
      `p.val. adj. (bf=7)` < 0.001,
      as.character(scientific(`p.val. adj. (bf=7)`, digits=2)),
      as.character(round(`p.val. adj. (bf=7)`, 4))

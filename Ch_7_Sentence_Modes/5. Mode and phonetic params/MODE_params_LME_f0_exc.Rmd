...
f0_exc_model <-
    lmer(
        f0_exc ~ mode + fin_phon + (1 + mode | speaker) + (1 | gender),
        data = m_corpus,
        control = lmerControl(
            optimizer = "optimx",
            calc.derivs = FALSE,
...
### `f0_exc ~ mode + fin_phon + prompt + (1 + mode | speaker)`

...
```
- Singularity issue with maximal model.
- Running model without gender as a random intercept.
- This fixes the issue.

### `lh_slope ~  mode + fin_phon + prompt + (1 + mode | speaker)`

```{r run reduced model,  fig.height=3, fig.width=9, tidy = TRUE}
...
charts <- getModelFixedFX(
  my_equation = lh_slope ~  mode + fin_phon + prompt + (1 + mode | speaker),
  my_data = m_corpus,

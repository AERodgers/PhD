...
- Initial model works well, but trimming observation where residual standard deviation > 3.5

## `l_t ~ mode + fin_phon + prompt + (1 + mode | speaker) + (1 | gender)`
...
# run model
l_t_model <-
  lmer(l_t ~ mode + fin_phon + prompt + (1 + mode | speaker) + (1 | gender),
       data = m_corpus,
        control = lmerControl(
          optimizer = "optimx",
          calc.derivs = FALSE,
...
l_t_model.trimmed <- lmer(l_t ~ mode + fin_phon + prompt + (1 + mode | speaker) + (1 | gender),
...
charts <- getModelFixedFX(
  my_equation = l_t ~ mode + fin_phon + prompt + (1 + mode | speaker) + (1 | gender),

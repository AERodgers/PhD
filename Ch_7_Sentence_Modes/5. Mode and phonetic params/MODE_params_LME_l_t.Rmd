...
# run model
l_t_model <-
  lmer(l_t ~ mode + fin_phon + gender + (1 + mode | speaker),
       data = m_corpus,
        control = lmerControl(
          optimizer = "optimx",
          calc.derivs = FALSE,
...
l_t_model.trimmed <- lmer(l_t ~ mode + fin_phon + gender + (1 + mode | speaker),
...
charts <- getModelFixedFX(
  my_equation = l_t ~ mode + fin_phon + gender + (1 + mode | speaker),
